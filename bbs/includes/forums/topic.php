...
if ($approved == '0'){
	nova_redirect("index.php?page=error&error=35","error/35");
}  
}

$query2167 = "select FORUM_ID, TITLE, LOCKED, STICKY, ANNOUNCE from {$db_prefix}posts WHERE TOPIC_ID='$topic' AND TITLE!=''" ;

$result2167 = mysql_query($query2167) or die("topic.php - Error in query: $query2167") ; 
$does_exist = mysql_num_rows($result2167);

if ($does_exist == '0'){
	nova_redirect("index.php?page=error&error=24","error/24");
}                                 
...
if ($can_view_forum=='0'){
	nova_redirect("index.php?page=error&error=2","error/2");
}
elseif($can_read_topics=='0'){
	nova_redirect("index.php?page=error&error=3","error/3");
}
else{
...
$query211 = "select MEMBER, TIME, FORUM_ID, CONTENT, ID, EDIT_TIME, EDIT_MEMBER, EDIT_REASON, TITLE, DESCRIPTION, STICKY, ANNOUNCE, REPORTED, APPROVED from {$db_prefix}posts WHERE TOPIC_ID='$topic' ORDER BY TIME asc LIMIT $limit, $list_posts";
}
else{
$query211 = "select MEMBER, TIME, FORUM_ID, CONTENT, ID, EDIT_TIME, EDIT_MEMBER, EDIT_REASON, TITLE, DESCRIPTION, STICKY, ANNOUNCE, REPORTED, APPROVED from {$db_prefix}posts WHERE TOPIC_ID='$topic' AND APPROVED='1' ORDER BY TIME asc LIMIT $limit, $list_posts";
}
...
$edit_form_title = $results211['TITLE'];
$edit_form_desc = $results211['DESCRIPTION'];
...
$topic_trackback_text = sprintf($lang['topic_trackback_text'], nova_link('index.php?page=findpost&post=' . $post_id, 'findpost/ ' . $post_id));
...
$query_custom1 = "select ID, NAME, ORDER_FIELD from {$db_prefix}custom_fields ORDER BY ORDER_FIELD asc, ID desc" ;
$result_custom1 = mysql_query($query_custom1) or die("topic.php - Error in query: $query_custom1") ;                                  
while ($results_custom1 = mysql_fetch_array($result_custom1)){
$custom_field_id = $results_custom1['ID'];
$custom_field_name = $results_custom1['NAME'];

$query_custom2 = "select CONTENT from {$db_prefix}custom_members WHERE MEMBER_ID='$id' AND FIELD_ID='$custom_field_id'" ;
$result_custom2 = mysql_query($query_custom2) or die("topic.php - Error in query: $query_custom2") ;                                  
while ($results_custom2 = mysql_fetch_array($result_custom2)){
$custom_profile_content = $results_custom2['CONTENT'];
