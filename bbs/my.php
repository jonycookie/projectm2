...
/*
	...
	$Id: my.php 11169 2007-11-08 06:30:00Z tiger $
*/
	...
	if(!submitcheck('buddysubmit', 1)) {
		...
		if(($newbuddy && $newbuddy != $discuz_userss) || ($newbuddyid && $newbuddyid != $discuz_uid)) {
			if(!in_array($adminid, array(1, 2, 3))) {
				if(($db->result_first("SELECT COUNT(*) FROM {$tablepre}buddys WHERE uid='$discuz_uid'")) > 20) {
					showmessage('buddy_add_toomany');
				}
			}

			if($buddyid = $db->result_first("SELECT uid FROM {$tablepre}members WHERE ".(empty($newbuddyid) ? "username='$newbuddy'" : "uid='$newbuddyid'"))) {
