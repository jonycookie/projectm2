...
if (isset($_POST['content'])){
	$query2 = "select ROLE from {$db_prefix}members WHERE NAME='$nova_name'" ;
...
if ($ext=='jpeg'){
			...
			$imgSx = imagesx($nova_domain.'/uploads/attachments/'.$filename);
			$imgSy = imagesy($nova_domain.'/uploads/attachments/'.$filename);
...
$attachment .= "<a href=\"".nova_link("download.php?attach=$row&filename=$original_filename", "download/$row/$original_filename")."\"><img src=\"$nova_domain/uploads/attachments/$filename\" alt=\"".$lang['topic_img']."\" /></a>";
...
$attachment .= "<div class=\"spacer\"></div>";
$attachment .= "<img src=\"$zip_icon_img\" alt=\"file\" /> <a class=\"post\" href=\"".nova_link("download.php?attach=$row&filename=$original_filename", "download/$row/$original_filename")."\">$original_filename</a><br />";
...
// Color (and colour for those of us that can spell)
	...
	$content = preg_replace("/\[color=(?:&quot;|\"|')?(\#[0-9A-F]{3,6}|[a-z]+)(?:&quot;|\"|')?\]/", '<span style="color:$1">', $content);
    $content = str_replace("[/color]", '</span>', $content);
		
	$content = preg_replace("/\[colour=(?:&quot;|\"|')?(\#[0-9A-F]{6}|[a-z]+)(?:&quot;|\"|')?\]/", '<span style="color:$1">', $content);
    $content = str_replace("[/colour]", '</span>', $content);
...
// Supersize me...
	...
	$content = preg_replace("#\[url\] " . $urlRegex . "\[/url\]#is", "<a class='post' href='$1'>$1</a>", $content);
	$content = preg_replace("#\[url=(?:&quot;|\"|')? " . $urlRegex . "(?:&quot;|\"|')?\](.*?)\[/url\]#is", "<a class='post' href='$1'>$2</a>", $content);
...
// Warn about Admin CP links	
	...
	$content = str_replace("href=\"$nova_domain/admin", "onclick=\"javascript: return confirm('".$lang['parse_url_warn']."')\" href=\"$nova_domain/admin", $content);	
	$content = str_replace("href=\"$nova_domain/index.php?page=admin", "onclick=\"javascript: return confirm('".$lang['parse_url_warn']."')\" href=\"$nova_domain/index.php?page=admin", $content);
	...
	if ($sef_urls=='1'){
		$content = preg_replace($expr, "<blockquote><div class='quote-name'>$1 <a href='$nova_domain/findpost/$2'><img src='$trackback_img' alt='Trackback URL' title='Trackback URL' /></a></div><div style='padding:2px;'>", $content);
	}
	else{
		$content = preg_replace($expr, "<blockquote><div class='quote-name'>$1 <a href='$nova_domain/index.php?page=findpost&post=$2'><img src='$trackback_img' alt='Trackback URL' title='Trackback URL' /></a></div><div style='padding:2px;'>", $content);
	}
...
// Is the post hidden from you?
	...
	if ($have_i_replied!='0'){
		...
		$content = preg_replace( "#\[hide\](.+?)\[/hide\]#is", "<div class='hide-tag'><div style='text-align: center;'><img src='$unhidden_img' alt='' /></div><div style='float: none; clear: both;'>$1</div></div>", $content );
	}
	else{
...
// Introduce smilies...
	...
	if (file_exists("themes/$theme/images/forums/emoticons")){
		$query219 = "select CODE, LINK from {$db_prefix}smilies WHERE EMOTICON_ON='1' AND CODE!='' AND LINK!='' AND THEME='$theme' ORDER BY ROW desc";
		$smiley_path="$nova_domain/themes/$theme/images/forums/emoticons";
	}
	else{
		$query219 = "select CODE, LINK from {$db_prefix}smilies WHERE EMOTICON_ON='1' AND CODE!='' AND LINK!='' AND THEME='default' ORDER BY ROW desc";
		$smiley_path="$nova_domain/images/forums/emoticons";
	}

	$result219 = mysql_query($query219) or die("topic.php - Error in query: $query219") ;                                  
	while ($results219 = mysql_fetch_array($result219)){
		$code = $results219['CODE'];
		$link = $results219['LINK'];
