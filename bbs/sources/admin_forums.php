...
/*
	...
	$Header: /cvsroot/usebb/UseBB/sources/admin_forums.php,v 1.25 2009/10/15 18:15:20 pc_freak Exp $
	...
	if ( !count($cats) ) {
		...
		if ( !empty($_POST['name']) && !empty($_POST['cat_id']) && array_key_exists($_POST['cat_id'], $cats) && !count($forum_moderators_unknown) ) {
			...
			$content .= '<table id="adminregulartable">';
				...
				$content .= '<tr><td class="fieldtitle">'.$lang['ForumsDescription'].'</td><td><textarea name="descr" rows="3" cols="50">'.unhtml(stripslashes($_POST['descr'])).'</textarea><div class="moreinfo">'.$lang['ForumsDescriptionExplain'].'</div></td></tr>';
