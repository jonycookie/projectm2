...
<?php } ?>				
				...
				<td class="forum-index-td-middle<?php echo "$alt_td_class"; ?>">
					<a class="forum-name-link" href="<?php echo nova_link("index.php?forum=$forum_id", "forum/$forum_title-$forum_id"); ?>"><strong><?php echo "$forum_name"; ?></strong></a><br /><?php echo "$forum_description"; ?>
<?php if ($count_sub!='0'){ ?>
...
<?php if ($count_sub_forums==$count_sub){ ?>
					<!-- show sub-forums -->
					&nbsp;<a class="subforum-name-link" href="<?php echo nova_link("index.php?forum=$sub_id", "forum/$forum_title-$sub_id"); ?>"><?php echo "$sub_name"; ?></a>
...
<?php }else{ ?>
					<?php echo $lang['board_topic_in']; ?> <a class="forum-index-link-to-topic" href="<?php echo nova_link("index.php?page=findpost&post=$id", "findpost/$id"); ?>"><?php echo "$title"; ?></a>
<?php } ?>
					<br />
...
<?php $pages=(ceil($number_of_threads/$list_topics)); if (!isset($_GET['limit'])){$limit="1";}else{$limit=$_GET['limit'];} $start="1"; for($i = "1"; $i <= $pages; $i++) { $i_limit=$i; if ($i > 1){ $start=($start+1); } if ($limit==$start){ ?>
				<a class="page-active" href="<?php echo nova_link("index.php?forum=$forum&limit=$limit", "forum/$forum_title-$forum/$limit"); ?>"><?php echo "$i"; ?></a>
<?php }else{ ?>
...
<?php if ($_GET['limit'] > "2" && $i_limit < $_GET['limit'] - "1"){ ?>
				<a class="page" href="<?php echo nova_link("index.php?forum=$forum&limit=1", "forum/$forum_title-$forum/1"); ?>">&lt;&lt;</a>
<?php } ?>
				<a class="page" href="<?php echo nova_link("index.php?forum=$forum&limit=$i_limit", "forum/$forum_title-$forum/$i_limit"); ?>"><?php echo "$i"; ?></a>
<?php }}} ?>
<?php if ($pages - $_GET['limit'] >= "3"){ ?>
				<a class="page" href="<?php echo nova_link("index.php?forum=$forum&limit=$pages_end", "forum/$forum_title-$forum/$pages_end"); ?>">&gt;&gt;</a>
<?php } ?>
			</td>
<?php } ?>
<?php if ($forum_read_only=='0'){ if ($can_add_topics=='1'){ ?>
			...
			<td class="new-topic-placer">
				<a class="submit-button-large img-add-forum" href="<?php echo nova_link("index.php?func=newpost&forum=$forum", "newpost/$forum"); ?>"><?php echo $lang['button_new_topic']; ?></a>
			</td>
		</tr></table>
<?php }else{ ?>
<?php }} ?>
...
<?php if ($show_rss=='1'){ ?>
				<a href="<?php echo "$nova_domain"; ?>/rss/rss.php?forum=<?php echo "$forum"; ?>"><img  src="<?php echo "$rss_img"; ?>" alt="<?php echo $lang['board_rss']; ?>" title="<?php echo $lang['board_rss']; ?>" /></a>&nbsp;&nbsp;
<?php } ?>
				<?php echo "$forum_name_for_this_forum"; ?>
			</td>
<?php if ($forum_read_only=='0'){ if ($_COOKIE['nova_name']!=''){ ?>
			<!-- show subscribe/unsubscribe option -->
			<td class="forum-topic-subscribe">
<?php if ($subscribed_already=='1'){ ?>
				<a class="subscribe-button img-unsubscribe" href="<?php echo nova_link("index.php?page=unsubscribe&forum=$forum", "unsubscribe/forum/$forum"); ?>">
					<?php echo $lang['button_unsubscribe_forum']; ?>
				</a>
<?php }else{ ?>
				<a class="subscribe-button img-subscribe" href="<?php echo nova_link("index.php?page=subscribe&forum=$forum", "subscribe/forum/$forum"); ?>">
...
<?php if ($can_lock_topics=='1' && $locked=='0'){ ?>
					<a href="<?php echo nova_link("index.php?func=lock&index=q&topic=$topic_id", "lock/q/$topic_id"); ?>">
<?php }elseif ($can_lock_topics=='1' && $locked=='1'){ ?>
					<a href="<?php echo nova_link("index.php?func=unlock&index=q&topic=$topic_id", "unlock/q/$topic_id"); ?>">
...
<?php if ($read_all_posts < $post_time && $read_time < $post_time){ ?>
					<a href="<?php echo nova_link("index.php?page=findpost&post=$post_to_be_read", "findpost/$post_to_be_read"); ?>"><img  src="<?php echo "$new_post_img"; ?>" alt="*" title="<?php echo $lang['board_forum_unread']; ?>" /></a>
<?php } ?>
...
<?php if ($sticky=='1'){if ($announce=='1'){ ?>
					...
					<a class="forum-index-topic-sticky" href="<?php echo nova_link("index.php?topic=$topic_id", "topic/$topic_title-$topic_id"); ?>"><?php echo "$title"; ?></a>
<?php }else{ ?>
					<a class="forum-index-topic-sticky" href="<?php echo nova_link("index.php?topic=$topic_id", "topic/$topic_title-$topic_id"); ?>"><?php echo "$title"; ?></a>
<?php }}else{ ?>
					<a class="topic-link-board" href="<?php echo nova_link("index.php?topic=$topic_id", "topic/$topic_title-$topic_id"); ?>"><?php echo "$title"; ?></a>
<?php } ?>
					<!-- topic description -->
...
<?php $pages=(ceil($number_of_posts/$list_posts)); if (!isset($_GET['limit'])){$_GET['limit']="1";} $start="1"; for($i = "1"; $i <= $pages; $i++) { $i_limit=$i; if ($i > 1){ $start=($start+1); } if ($_GET['limit']==$start){ ?>
						<a class="page-small" href="<?php echo nova_link("index.php?topic=$topic_id&limit=$i_limit", "topic/$topic_title-$topic_id/$i_limit"); ?>"><?php echo "$i"; ?></a>
<?php } else{ ?>
<?php if ($_GET['limit'] >="3" && $i_limit < $_GET['limit'] - "2" OR $_GET['limit'] >="3" && $i_limit > $_GET['limit'] + "2"){}elseif($_GET['limit'] >="3" && $i_limit > $_GET['limit'] + "2"){}elseif($_GET['limit'] <= "2" && $i_limit > "4" ){}else{  ?>
<?php if ($_GET['limit'] > "2" && $i_limit < $_GET['limit'] - "1"){ ?>
						<a class="page-small" href="<?php echo nova_link("index.php?topic=$topic_id&limit=1", "topic/$topic_title-$topic_id/1"); ?>">&lt;&lt;</a>
<?php } ?>
						<a class="page-small" href="<?php echo nova_link("index.php?topic=$topic_id&limit=$i_limit", "topic/$topic_title-$topic_id/$i_limit"); ?>"><?php echo "$i"; ?></a>
<?php }}} ?>
<?php if ($pages_end - $_GET['limit'] >= "3"){ ?>
						<a class="page-small" href="<?php echo nova_link("index.php?topic=$topic_id&limit=$pages_end", "topic/$topic_title-$topic_id/$pages_end"); ?>">&gt;&gt;</a>
<?php } ?>
<?php } ?>	
					</div>
				</td>
...
<?php } ?>
                ...
                <td class="forum-board-lastpost<?php echo "$alt_td_class"; ?><?php echo " $status_class"; ?>"><?php echo "$time"; ?><br />
					<a class="forum-index-link-to-topic" href="<?php echo nova_link("index.php?page=findpost&post=$last_post", "findpost/$last_post"); ?>"><span class="topic-description"><?php echo $lang['board_lastpost_by']; ?></a></a> 
