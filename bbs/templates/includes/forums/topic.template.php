...
<?php $pages=(ceil($number_of_posts/$list_posts)); if (!isset($_GET['limit'])){$_GET['limit']="1";} $start="1"; for($i = "1"; $i <= $pages; $i++) { $i_limit=$i; if ($i > 1){ $start=($start+1); } if ($_GET['limit']==$start){ ?>
					<a class="page-active" href="<?php echo nova_link("index.php?topic=$topic&limit=$i_limit", "topic/$topic_title-$topic/$i_limit"); ?>"><?php echo "$i"; ?></a>
<?php } else{ ?>
...
<?php if ($_GET['limit'] > "2" && $i_limit < $_GET['limit'] - "1"){ ?>
					<a class="page" href="<?php echo nova_link("index.php?topic=$topic&limit=1", "topic/$topic_title-$topic/1"); ?>">&lt;&lt;</a>
<?php } ?>
					<a class="page" href="<?php echo nova_link("index.php?topic=$topic&limit=$i_limit", "topic/$topic_title-$topic/$i_limit"); ?>"><?php echo "$i"; ?></a>
<?php }}} ?>
<?php if ($pages - $_GET['limit'] >= "3"){ ?>
					<a class="page" href="<?php echo nova_link("index.php?topic=$topic&limit=$pages_end", "topic/$topic_title-$topic/$pages_end"); ?>">&gt;&gt;</a>
<?php } ?>
				</td>
<?php } ?>
...
<?php if ($read_only=='1'){} elseif($can_add_topics=='1'){ ?>
					<a class="submit-button-large img-add-forum" href="<?php echo nova_link("index.php?func=newpost&forum=$forum_id", "newpost/$forum_id"); ?>"><?php echo $lang['button_new_topic']; ?></a>
<?php } if ($locked=='1'){} elseif($can_reply_topics=='1'){ ?>
					<a class="submit-button-large img-add-reply" href="<?php echo nova_link("index.php?func=addreply&topic=$topic", "addreply/$topic"); ?>"><?php echo $lang['button_add_reply']; ?></a>
...
<?php if ($locked=='0'){ if ($nova_name!=''){ ?>
				<td class="forum-topic-subscribe">
<?php if ($subscribed_already=='1'){ ?>
					<a class="subscribe-button img-unsubscribe" href="<?php echo nova_link("index.php?page=unsubscribe&topic=$topic", "unsubscribe/topic/$topic"); ?>"><?php echo $lang['button_unsubscribe_topic']; ?></a>
<?php }else{ ?>
					<a class="subscribe-button img-subscribe" href="<?php echo nova_link("index.php?page=subscribe&topic=$topic", "subscribe/topic/$topic"); ?>"><?php echo $lang['button_subscribe_topic']; ?></a>
<?php } ?>
				</td>
<?php }} ?>
			</tr>
...
<?php } elseif($template_hook=='6'){ ?>
		<!--  polls form -->
		<form method="post" action="<?php echo nova_link("index.php?action=vote&topic=$topic", "vote/$topic"); ?>">
...
<?php } ?>
					...
					<div class="center">
						...
						<a class="submit-button img-submit" href="<?php echo nova_link("index.php?topic=$topic&limit=1&showresults=1", "topic/$topic_title-$topic/1/showresults"); ?>"><?php echo $lang['topic_poll_results']; ?></a>
					</div>
					<hr />
...
<?php }elseif ($template_hook=='10'){ ?>
							...
							<div class="center"><br /><br />
								<a class="submit-button img-poll" href="<?php echo nova_link("index.php?topic=$topic", "topic/$topic_title-$topic"); ?>"><?php echo $lang['button_poll']; ?></a>
							</div>
...
<?php }elseif ($template_hook=='14'){ ?>
					...
					<tr class="topic-trackback-text" style="display: none" id="trackback_url_<?php echo $post_id; ?>">
						<td class="forum-board-trackback-left"></td>
						<td class="forum-board-trackback">
...
<?php if ($title==''){ if ($can_split_topics=='1'){ ?>
								<a class="submit-button img-split" href="<?php echo nova_link("index.php?func=split&post=$post_id", "split/$post_id"); ?>"><?php echo $lang['button_split']; ?></a>
<?php }} ?>
<?php if ($title!=''){ if ($can_move_topics=='1'){ ?>
								<a class="submit-button img-move" href="<?php echo nova_link("index.php?func=move&topic=$topic", "move/$topic"); ?>"><?php echo $lang['button_move']; ?></a>
<?php } if ($can_merge_topics=='1'){ ?>
								<a class="submit-button img-merge" href="<?php echo nova_link("index.php?func=merge&topic=$topic", "merge/$topic"); ?>"><?php echo $lang['button_merge']; ?></a>
<?php }} if ($title!='' && $locked=='0'){ if ($can_lock_topics=='1'){ ?>
								<a class="submit-button img-lock" href="<?php echo nova_link("index.php?func=lock&topic=$topic", "lock/$topic"); ?>"><?php echo $lang['button_lock']; ?></a>
<?php }} elseif ($title!='' && $locked=='1'){ if ($can_lock_topics=='1'){ ?>
								<a class="submit-button img-unlock" href="<?php echo nova_link("index.php?func=unlock&topic=$topic", "unlock/$topic"); ?>"><?php echo $lang['button_unlock']; ?></a>
<?php }} ?>
							</span>
						</td>
					</tr>
...
<?php if ($member_role!='1'){ if ($can_warn_members=='1'){ ?>
							<a class="warn-minus" href="<?php echo nova_link("index.php?page=warn&act=minus&id=$id", "warn/minus/$id"); ?>" title="<?php echo $lang['topic_warn_remove']; ?>">&nbsp;</a><a class="warn-mods warn-level<?php echo "$graphic_level"; ?>" href="javascript:WarnPopUp('<?php echo "$nova_domain"; ?>/includes/forums/warn_popup.php?id=<?php echo "$id"; ?>')" title="<?php echo $lang['topic_warn_level']; ?> <?php echo "$graphic_warn"; ?>%">&nbsp;</a><a class="warn-plus" href="<?php echo nova_link("index.php?page=warn&act=add&id=$id", "warn/add/$id"); ?>" title="<?php echo $lang['topic_warn_add']; ?>">&nbsp;</a>
							<br />
<?php }else{ ?>
							<!-- warn bar for non-mods -->
							<a class="warn-members warn-level<?php echo "$graphic_level"; ?>" href="javascript:WarnPopUp('<?php echo "$nova_domain"; ?>/includes/forums/warn_popup.php?id=<?php echo "$id"; ?>')" title="<?php echo $lang['topic_warn_level']; ?> <?php echo "$graphic_warn"; ?>%">&nbsp;</a>
							<br />
<?php }} ?>
...
<?php } ?>
								<a href="<?php echo nova_link("download.php?attach=$row&filename=$original_filename", "download/$row/$original_filename"); ?>"><img  src="<?php echo "$nova_domain"; ?>/uploads/attachments/<?php echo "$filename"; ?>" alt="<?php echo $lang['topic_img']; ?>" /></a>
								<br /><strong><?php echo "$downloads"; ?></strong> <?php echo $lang['topic_downloads']; ?>
							</div>
<?php }elseif ($template_hook=='19'){ ?>
							<!-- file attachment -->
							<div style="width: 100%; clear: both;">
								<img  src="<?php echo "$zip_icon_img"; ?>" alt="file" /> <a class="post" href="<?php echo nova_link("download.php?attach=$row&filename=$original_filename", "download/$row/$original_filename"); ?>"><?php echo "$original_filename"; ?></a><br />
...
<?php if ($can_pm=='1'){ if ($can_pm_this_member=='1'){ ?>
							<a class="submit-button img-email-go" href="<?php echo nova_link("index.php?page=messages&act=new&id=$id", "messages/new/$id"); ?>"><?php echo $lang['button_send_pm']; ?></a>
<?php }} ?>
						</td>
						<td class="forum-board-post forum-board-post-bottom">&nbsp;
							<!-- admin links -->
<?php if ($locked!='1'){ if ($can_reply_topics=='1'){ ?>
							<a class="submit-button img-add-reply" href="<?php echo nova_link("index.php?func=addreply&topic=$topic&quote=$post_id", "addreply/$topic/$post_id"); ?>"><?php echo $lang['button_reply']; ?></a>
							<a class="submit-button img-quote-on" id="mad_<?php echo "$post_id"; ?>" name="mad_<?php echo "$post_id"; ?>" href="#" onclick="novaquote(<?php echo "$post_id"; ?>); return false;"><?php echo $lang['button_quote']; ?></a>
...
<?php } ?>
							<a class="submit-button img-edit" href="<?php echo nova_link("index.php?func=edit&post=$post_id", "edit/$post_id"); ?>"><?php echo $lang['button_edit']; ?></a>
...
<?php }}} if (($can_delete_others_posts == 1 || ($member == $my_id && $can_delete_own_posts == 1)) && $locked != 1){ ?>
							...
							<form method="post" action="<?php echo nova_link('index.php?func=del&post=' . $post_id, 'del/' . $post_id); ?>">
								...
								<input
									...
									onclick="javascript:return confirm('a<?php echo $lang['topic_remove']; ?>')"
								/>
							
							</form>

<?php } if ($nova_name!=''){ if ($can_warn_members=='0'){ ?>
							<a class="submit-button img-error" href="<?php echo nova_link("index.php?page=report&post=$post_id", "report/$post_id"); ?>"><?php echo $lang['button_report']; ?></a>
<?php }} ?>
<?php if ($can_moderate_members=='1' && $approved=='0'){ ?>
							<a class="submit-button button-update img-error" href="<?php echo nova_link("index.php?page=admin&act=preview&action=approve&id=$moderate_post&post=$post_id", "admin/preview/approve/$moderate_post/$post_id"); ?>"><?php echo $lang['button_approve']; ?></a>
<?php } ?>
						</td>
					</tr>
...
<?php if ($quick_edit=='1'){ ?>
					<tr>
						...
						<td class="forum-board-edit-right forum-index-bottom">
							<div class="<?php echo "$slide-state"; ?>" id="edit_<?php echo "$post_id"; ?>" style="display: none;">
								<div style="padding: 8px;">
									<form name="editcontent" method="post" action="<?php echo nova_link("index.php?func=edit","edit"); ?>">
										<!-- textarea -->
...
<?php }elseif ($template_hook=='25'){ ?>
					...
					<tr>
						<td class="forum-board-quick-edit-left forum-index-top" style="padding: 5px;">
							<a class="forum-header" href="<?php echo nova_link("index.php?topic=$previous_topic_id", "topic/$previous_title-$previous_topic_id"); ?>""><?php echo $lang['topic_previous']; ?></a>
							&nbsp;&nbsp;<a class="forum-header" href="<?php echo nova_link("index.php?topic=$next_topic_id", "topic/$next_title-$next_topic_id"); ?>""><?php echo $lang['topic_next']; ?></a>
						</td>
						<td class="forum-board-quick-edit-right forum-index-top" style="padding: 5px; text-align: right;">
							<form method="get" name="searchform" action="<?php echo "$nova_domain"; ?>/index.php?">
							...
							<td class="forum-jump-content" style="width:100%;">
								<div class="smilies" style="position: relative;">
									...
									<div class="smilies-content">
										...
										<br />(<a href="javascript:SmiliesPopUp('<?php echo "$nova_domain"; ?>/includes/forums/smilies_popup.php')"><?php echo $lang['addreply_emoticons']; ?></a>)
									</div>
								</div>
								<div class="textarea-addreply">
									<form method="post" name="postcontent" action="<?php echo nova_link("index.php?func=addreply", "addreply"); ?>">
...
<?php } ?>
									...
									<form name="preview" action="<?php echo "$nova_domain"; ?>/includes/forums/preview.php" method="post" target="preview">
					...
					<script language="javascript">
						function copydata() {
							...
							window.open('<?php echo "$nova_domain"; ?>/includes/forums/preview.php', 'preview', 'height=400,width=700,scrollbars=yes');
						}
					</script>		
				</div>
...
<?php }elseif($template_hook=='34'){ ?>
					<tr>
						...
						<td class="forum-board-subject">
							<a href="<?php echo nova_link("index.php?page=history&post=$post_id&entry=$edited_row", "history/$post_id/$edited_row"); ?>"  onclick="javascript:return confirm('<?php echo $lang['topic_remove']; ?>')"><img  src="<?php echo "$delete_icon_img"; ?>" alt="" /></a>&nbsp;
