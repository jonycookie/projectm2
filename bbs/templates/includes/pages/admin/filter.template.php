...
<?php }elseif ($template_hook=="3"){ ?>
		...
		<form name="settings" method="post" action="<?php echo nova_link("index.php?page=admin&act=filter","admin/filter"); ?>">
...
<?php }elseif ($template_hook=="4"){ ?>
					...
					<div style="width: 5%; float: left; vertical-align: middle;">
						<a href="<?php echo nova_link("index.php?page=admin&act=filter&id=$row","admin/filter/$row"); ?>" onclick="javascript:return confirm('<?php echo $lang['topic_remove']; ?>')"><img  src="<?php echo "$delete_icon_img"; ?>" alt="<?php echo $lang_admin['custom_delete']; ?>" /></a>
					</div>
