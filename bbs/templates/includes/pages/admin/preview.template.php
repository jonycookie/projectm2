...
<?php }elseif ($template_hook=="3"){ ?>
					<div style="width: 10%; float: left;">
						<a href="<?php echo nova_link("index.php?page=findpost&post=$post","findpost/$post"); ?>">#<?php echo "$post"; ?></a>
					</div>
					<div style="width: 20%; float: left;">
						<a class="forum-index-link-to-topic" href="<?php echo nova_link("index.php?page=messages&act=new&id=$member_id","messages/new/$member_id"); ?>" title="<?php echo $lang_admin['preview_message']; ?>"><span class="warn-remove"><?php echo "$member_name"; ?></span></a>
					</div>
					<div style="width: 50%; float: left;">
						<?php echo "$title"; ?>
					</div>
					<div style="width: 20%; float: left; text-align: center;">
						<a href="<?php echo nova_link("index.php?page=admin&act=preview&action=approve&id=$id","admin/preview/approve/$id"); ?>"><img  src="<?php echo "$plus_img"; ?>" alt="tick" title="<?php echo $lang_admin['preview_approve']; ?>" /></a> 
						<a href="<?php echo nova_link("index.php?page=admin&act=preview&action=reject&id=$id","admin/preview/reject/$id"); ?>"><img  src="<?php echo "$minus_img"; ?>" alt="tick" title="<?php echo $lang_admin['preview_reject']; ?>" /></a>
					</div>
