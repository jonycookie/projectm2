...
<?php }elseif($template_hook=="2"){ ?>
		...
		<form name="settings" method="post" action="<?php echo nova_link("index.php?page=admin&act=smilies","admin/smilies"); ?>">
...
<?php }elseif($template_hook=="3"){ ?>
					<div style="width: 20%; float: left;">
						<img  src="<?php echo "$nova_domain"; ?>/<?php echo "$emoticon_path"; ?>/<?php echo "$file"; ?>" alt="<?php echo "$file"; ?>" />
					</div>
...
<?php }else{ ?>
						<input type="text" name="emoticon<?php echo "$smiley_count"; ?>" value="<?php echo "$code"; ?>" size="10" /><br /><br />
<?php } ?>
					</div>
					<div style="width: 15%; float: right; text-align: right;">
						<?php echo $lang_admin['smilies_on']; ?> 
<?php if ($link==$file && $emoticon_on=='1'){ ?>
...
<?php } ?>
						<input type="hidden" value="<?php echo "$file"; ?>" name="file<?php echo "$smiley_count"; ?>" />
					</div>
		...
		<table class="forum-index" cellpadding="0" cellspacing="0">
			...
			<tr><td class="forum-jump-content forum-index-top forum-index-bottom">
				...
				<form method="post" action="index.php?">
					<select name="switch" onchange="location.href='<?php echo nova_link("index.php?page=admin&act=smilies&location=","admin/smilies/"); ?>'+(this.options[this.selectedIndex].value)">
