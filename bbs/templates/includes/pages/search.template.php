...
<?php $pages=(ceil($number_of_posts/$list_posts)); if (!isset($_GET['limit'])){$_GET['limit']="1";} $start="1"; for($i = "1"; $i <= $pages; $i++) { $i_limit=$i; if ($i > 1){ $start=($start+1); } if ($_GET['limit']==$start){ ?>
		<a class="page-active" href="<?php echo "$nova_domain"; ?>/index.php?page=search&area=topics&search=<?php echo "$member_id"; ?>&limit=<?php echo "$i_limit"; ?>"><?php echo "$i"; ?></a><?php } else{ ?><?php if ($_GET['limit'] >="3" && $i_limit < $_GET['limit'] - "2" OR $_GET['limit'] >="3" && $i_limit > $_GET['limit'] + "2"){}elseif($_GET['limit'] >="3" && $i_limit > $_GET['limit'] + "2"){}elseif($_GET['limit'] <= "2" && $i_limit > "4" ){}else{  ?><?php if ($_GET['limit'] > "2" && $i_limit < $_GET['limit'] - "1"){ ?>
		<a class="page" href="<?php echo "$nova_domain"; ?>/index.php?page=search&area=topics&search=<?php echo "$member_id"; ?>&limit=1">&lt;&lt;</a><?php } ?>
		<a class="page" href="<?php echo "$nova_domain"; ?>/index.php?page=search&area=topics&search=<?php echo "$member_id"; ?>&limit=<?php echo "$i_limit"; ?>"><?php echo "$i"; ?></a><?php }}} ?>
<?php if ($pages - $_GET['limit'] >= "3"){ ?>
		<a class="page" href="<?php echo "$nova_domain"; ?>/index.php?page=search&area=topics&search=<?php echo "$member_id"; ?>&limit=<?php echo "$pages_end"; ?>">&gt;&gt;</a><?php } ?>
		</div>
...
<?php }elseif ($template_hook=='5'){ ?>
		...
		<div class="search-contents">
			<a class="search-title" href="<?php echo nova_link("index.php?page=findpost&post=$post_id","findpost/$post_id"); ?>"><?php echo "$title"; ?></a>
			<br />
			<div class="search-content"><span class="search-date"><?php echo "$time"; ?> - </span><?php echo "$content"; ?></div>
			<span class="search-link"><?php echo nova_link("index.php?page=findpost&post=$post_id","findpost/$post_id"); ?></span>
		</div>
...
<?php $pages=(ceil($number_of_posts/$list_posts)); if (!isset($_GET['limit'])){$_GET['limit']="1";} $start="1"; for($i = "1"; $i <= $pages; $i++) { $i_limit=$i; if ($i > 1){ $start=($start+1); } if ($_GET['limit']==$start){ ?>
		...
		<a class="page" href="<?php echo "$nova_domain"; ?>/index.php?page=search&area=posts&search=<?php echo "$member_id"; ?>&limit=0">&lt;&lt;</a><?php } ?>
		<a class="page" href="<?php echo "$nova_domain"; ?>/index.php?page=search&area=posts&search=<?php echo "$member_id"; ?>&limit=<?php echo "$i_limit"; ?>"><?php echo "$i"; ?></a><?php }}} ?>
<?php if ($pages - $_GET['limit'] >= "3"){ ?>
		<a class="page" href="<?php echo "$nova_domain"; ?>/index.php?page=search&area=posts&search=<?php echo "$member_id"; ?>&limit=<?php echo "$pages_end"; ?>">&gt;&gt;</a><?php } ?>
		</div>
<?php } ?>
...
<?php } ?>
			...
			<table class="forum-index" cellpadding="0" cellspacing="0">
				...
				<tr><td class="forum-jump-content forum-index-top forum-index-bottom">
					...
					<form method="get" action="<?php echo "$nova_domain"; ?>/index.php?">
...
<?php $pages=(ceil($number_of_posts/$list_posts)); if (!isset($_GET['limit'])){$_GET['limit']="1";} $start="1"; for($i = "1"; $i <= $pages; $i++) { $i_limit=$i; if ($i > 1){ $start=($start+1); } if ($_GET['limit']==$start){ ?>
		...
		<a class="page" href="<?php echo "$nova_domain"; ?>/index.php?page=search&author=<?php echo "$author"; ?>&author_id=<?php echo "$author_id"; ?>&startdate=<?php echo "$page_start"; ?>&enddate=<?php echo "$enddate_form"; ?>&forums[]=<?php echo "$forum_form"; ?>&search=<?php echo "$words"; ?>&pf=1&limit=1">&lt;&lt;</a><?php } ?>
		<a class="page" href="<?php echo "$nova_domain"; ?>/index.php?page=search&author=<?php echo "$author"; ?>&author_id=<?php echo "$author_id"; ?>&startdate=<?php echo "$page_start"; ?>&enddate=<?php echo "$enddate_form"; ?>&forums[]=<?php echo "$forum_form"; ?>&search=<?php echo "$words"; ?>&pf=1&limit=<?php echo "$i_limit"; ?>"><?php echo "$i"; ?></a><?php }}} ?>
<?php if ($pages - $_GET['limit'] >= "3"){ ?>
		<a class="page" href="<?php echo "$nova_domain"; ?>/index.php?page=search&author=<?php echo "$author"; ?>&author_id=<?php echo "$author_id"; ?>&startdate=<?php echo "$page_start"; ?>&enddate=<?php echo "$enddate_form"; ?>&forums[]=<?php echo "$forum_form"; ?>&search=<?php echo "$words"; ?>&pf=1&limit=<?php echo "$pages_end"; ?>">&gt;&gt;</a><?php } ?>
		</div>
<?php } ?>
...
<?php if ($pages <= '1'){}else{ ?>
		...
		<?php echo $lang['board_page']; ?>
<?php $pages=(ceil($number_of_posts/$list_posts)); if (!isset($_GET['limit'])){$_GET['limit']="1";} $start="1"; for($i = "1"; $i <= $pages; $i++) { $i_limit=$i; if ($i > 1){ $start=($start+1); } if ($_GET['limit']==$start){ ?>
		<a class="page-active" href="<?php echo nova_link("index.php?page=search&area=newposts&limit=$i_limit", "newposts/$i_limit"); ?>"><?php echo "$i"; ?></a><?php } else{ ?><?php if ($_GET['limit'] >="3" && $i_limit < $_GET['limit'] - "2" OR $_GET['limit'] >="3" && $i_limit > $_GET['limit'] + "2"){}elseif($_GET['limit'] >="3" && $i_limit > $_GET['limit'] + "2"){}elseif($_GET['limit'] <= "2" && $i_limit > "4" ){}else{  ?><?php if ($_GET['limit'] > "2" && $i_limit < $_GET['limit'] - "1"){ ?>
		<a class="page" href="<?php echo nova_link("index.php?page=search&area=newposts&limit=1", "newposts/1"); ?>">&lt;&lt;</a><?php } ?>
		<a class="page" href="<?php echo nova_link("index.php?page=search&area=newposts&limit=$i_limit", "newposts/$i_limit"); ?>"><?php echo "$i"; ?></a><?php }}} ?>
<?php if ($pages - $_GET['limit'] >= "3"){ ?>
		<a class="page" href="<?php echo nova_link("index.php?page=search&area=newposts&limit=$pages_end", "newposts/$pages_end"); ?>">&gt;&gt;</a><?php } ?>
		</div>
<?php } ?>
...
<?php } ?>
		<script type="text/javascript">
			var options = {
			script:"<?php echo "$nova_domain"; ?>/scripts/php/autosuggest.php?json=true&limit=6&",
			varname:"input",
			json:true,
			shownoresults:true,
			maxresults:1000,
			...
			var options_xml = {
			script: function (input) { return "<?php echo "$nova_domain"; ?>/scripts/php/autosuggest.php?input="+input+"&member_to="+document.getElementById('author_id').value; },
			varname:"input"
			};
