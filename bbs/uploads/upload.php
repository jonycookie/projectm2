...
$nova_domain 	= str_replace('/uploads/upload.php', '', $my_address); 	// returns http://myforum.com/forum style address
...
$query211 = "select ID, BANNED from {$db_prefix}members WHERE name='$nova_name' AND password='$nova_password'" ;
...
if ($my_id < '1' OR $banned=='1' OR !isset($nova_name) OR $id_count!='1'){
...
$query_theme = "select THEME from {$db_prefix}members WHERE NAME='$nova_name'" ;
...
<?php
if ($theme!='novaboard_default'){
echo "<link rel='stylesheet' href='$nova_domain/themes/novaboard_default/stylesheet.css' type='text/css'>";
}
echo "<link rel='stylesheet' href='$nova_domain/themes/$theme/stylesheet.css' type='text/css'>";
...
// if the user is logged in, let's get their ID, and if that doesn't work, return an error....
$query211 = "select ID from {$db_prefix}members WHERE name='$nova_name' AND password='$nova_password'" ;
$result211 = mysql_query($query211) or die("Query failed") ;                                  
...
echo "<meta http-equiv=\"refresh\" content=\"0;url=$nova_domain/uploads/upload.php?topicid=$topicid&attachtype=$attachtype&hash=$hash\" />";
echo "</div>";
...
*/
	...
	$query = mysql_query('SELECT id FROM ' . $db_prefix . 'members WHERE name = "' . $nova_name . '" AND password = "' . $nova_password . '"');
...
if ($avatar=='' OR $avatar==$nova_domain){
...
<select name="fileselect" onchange="form_submit(); location.href='<?php echo "$nova_domain"; ?>/uploads/upload.php?&downloadselect=1&topicid=<?php echo "$topicid"; ?>&attachtype=<?php echo "$attachtype"; ?>&member=<?php echo "$member"; ?>&hash=<?php echo "$hash"; ?>&id='+escape(this.options[this.selectedIndex].value)">
<?php
