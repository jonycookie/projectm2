...
/*
	...
	$Id: cp_poll.php 12768 2009-07-20 06:11:08Z liguode $
*/
...
if(submitcheck('pollsubmit')) {
	...
	foreach($_POST['option'] as $key => $val) {
		...
		if($option && count($newoption) < $maxoption) {
			$newoption[] = $option;
			if(count($preview) < 2 ) {
				$preview[] = $option;
			}
	...
	//判断是否有悬常
	if($_POST['credit']) {
		$_SGLOBAL['db']->query("UPDATE ".tname('space')." SET credit=credit-$_POST[credit] WHERE uid='$_SGLOBAL[supe_uid]'");
	}
	...
	if(empty($space['pollnum'])) {
		...
		$pollnumsql = "pollnum=".($space['pollnum']+1);
	} else {
		$pollnumsql = 'pollnum=pollnum+1';
	}
	//积分
	$reward = getreward('createpoll', 0);
	$_SGLOBAL['db']->query("UPDATE ".tname('space')." SET {$pollnumsql}, lastpost='$_SGLOBAL[timestamp]', updatetime='$_SGLOBAL[timestamp]', credit=credit+$reward[credit], experience=experience+$reward[experience] WHERE uid='$_SGLOBAL[supe_uid]'");
...
if($op == 'addopt') {
	...
	if(submitcheck('summarysubmit')) {
		...
		$summary = getstr(trim(str_replace(array("\r\n", "\n", "\r"), "<br/>", $_POST['summary'])), 0, 1, 1, 1, 2, 1);
	...
	if(submitcheck('votesubmit')) {
		...
		$setarr = array(
			...
			'pid' => $pid,
			'option' => saddslashes('"'.implode('"、"', $list).'"'),
			'dateline' => $_SGLOBAL['timestamp']
		);
