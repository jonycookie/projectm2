...
/*
	...
	$Id: cp_thread.php 12766 2009-07-20 04:26:21Z liguode $
*/
...
if(submitcheck('threadsubmit')) {
	...
	if(empty($_POST['tid'])) {
		...
		if(empty($space['threadnum'])) {
			...
			$threadnumsql = "threadnum=".($space['threadnum']+1);
		} else {
		...
		if(checkperm('edittrail') || ($thread['uid'] && $thread['uid'] != $space['uid'])) {
	...
	if($_POST['topicid']) {
		...
		$url = 'space.php?do=topic&topicid='.$_POST['topicid'].'&view=thread';
	} else {
