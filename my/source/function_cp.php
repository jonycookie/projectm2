...
/*
	...
	$Id: function_cp.php 12788 2009-07-21 01:05:22Z liguode $
*/
...
function pic_save($FILE, $albumid, $title, $topicid=0) {
	...
	if(function_exists('getimagesize')) {
		$imagesize = @getimagesize($new_name);
		list($width, $height, $type) = (array)$imagesize;
		$size = $width * $height;
		if($size > 16777216 || $size < 4 || empty($type)) {
			@unlink($new_name);
			return cplang('only_allows_upload_file_types');
		}
	}
...
function album_creat($arr) {
	...
	if($albumid) {
		...
		if(empty($space['albumnum'])) {
			...
			updatetable('space', array('albumnum'=>$space['albumnum']+1), array('uid'=>$space['uid']));
		} else {
			$_SGLOBAL['db']->query("UPDATE ".tname('album')." SET albumnum=albumnum+1 WHERE uid='$space[uid]'");
		}
...
function addfriendnum($touid, $tousername) {
	...
	//自己
	if($touid == $_SGLOBAL['supe_uid']) return false;
...
//参与热闹
function topic_join($topicid) {
	global $_SGLOBAL;
	
	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('topicuser')." WHERE uid='$_SGLOBAL[supe_uid]' AND topicid='$topicid'");
	if($value = $_SGLOBAL['db']->fetch_array($query)) {
