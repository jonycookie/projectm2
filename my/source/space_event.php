...
if($eventid){// 显示活动内容
	...
	//相关热点
	if(!$topic = topic_get($event['topicid'])) {
		updatetable('event', array('topicid'=>0), array('topicid'=>$blog['topicid']));
	}
	...
	if($view=="recommend") {
		...
		$sql .= ' ORDER BY eventid DESC LIMIT 0, 50';
	...
	if($count){
		if($needquery) {
			$selectsql = $view=="friend" ? "e.*, ue.uid as fuid, ue.username as fusername, ue.status" : "e.*";
			$sql = "SELECT $selectsql FROM $fromsql $joinsql WHERE ".implode(" AND ", $wherearr) ." ORDER BY $orderby LIMIT $start, $perpage";
		}
		$query = $_SGLOBAL['db']->query($sql);
