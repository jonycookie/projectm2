...
/*
	...
	$Id: admin_models.php 13342 2009-09-16 05:43:20Z zhaofei $
*/
...
EOF;
	...
	if(file_exists($datadir.'model.cache.php')) {	//检查主要sql文件是否存在
		...
		if(!empty($modelsql['categories'])) {
			$insertvalue = '';
			$insertpre = '';
			$modelsql['categories'] = saddslashes(shtmlspecialchars($modelsql['categories']));
			foreach($modelsql['categories'] as $tmpvalue) {
				$insertvalue .= $insertpre.'(\''.$tmpvalue['catid'].'\', \''.$tmpvalue['upid'].'\', 
									\''.$tmpvalue['name'].'\', \''.$tmpvalue['note'].'\', 
									\''.$tmpvalue['displayorder'].'\', \''.$tmpvalue['url'].'\', 
									\''.$tmpvalue['subcatid'].'\')';
				$insertpre = ',';
			}
			$_SGLOBAL['db']->query('INSERT INTO '.tname($_POST['modelname'].'categories').'  
								(catid, upid, name, note, displayorder, url, subcatid) VALUES '.$insertvalue);
		}
