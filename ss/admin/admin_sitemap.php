...
/*
	...
	$Id: admin_sitemap.php 13376 2009-09-25 06:02:55Z zhaofei $
*/
...
if(empty($_GET['op'])) {
	...
	if(is_dir($sitemap_path) && is_writable($sitemap_path)){
		...
		if($value = $_SGLOBAL['db']->fetch_array($query)) {
			...
			$i = $n = 0;
			$query = $_SGLOBAL['db']->query("SELECT max(itemid) FROM ".tname('spaceitems')." WHERE type='news'");
			$countnum =$_SGLOBAL['db']->result($query, 0);
			$query = $_SGLOBAL['db']->query("SELECT si.itemid, si.type, si.uid, si.dateline, si.subject, si.username, sn.message FROM ".tname('spaceitems')." si, ".tname('spacenews')." sn WHERE si.type='news' AND si.itemid = sn.itemid AND si.itemid > $value[lastitemid] ".$limit);
