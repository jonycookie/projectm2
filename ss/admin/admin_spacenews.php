...
/*
	...
	$Id: admin_spacenews.php 13411 2009-10-22 03:13:01Z zhaofei $
*/
...
if (submitcheck('listvaluesubmit')) {
	...
	if(!empty($_POST['operation'])) {
		...
		adminlog('admin_'.$_POST['operation'], $newidarr);
	}
	...
	if(empty($itemid) || $page == 1) {
		...
		if(!$_SGET['folder']) {
			...
			$showurlarr[] = array(CPURL.'?action=spacenews&op=view&itemid='.$pitemid, $alang['spacenews_newspage']);
			$showurlarr[] = array(geturl('action/category/catid/'.$_POST['catid'].'/php/1'), $alang['spacenews_typepage']);
		}
