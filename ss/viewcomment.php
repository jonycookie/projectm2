...
/*
	...
	$Id: viewcomment.php 13359 2009-09-22 09:06:19Z zhaofei $
*/
...
if(submitcheck('submitcomm', 1)) {
	...
	if($comment['floornum']) {
		...
		if (!$comment['hidelocation']) $html .= $comment['iplocation']!='LAN' ? $comment['iplocation'] : '火星';
		$html .= '网友';
		if (!empty($comment['authorid']) && !$comment['hideauthor']) $html .= " [{$comment['author']}] ";
		if (!$comment['hideip']) $html .= " ({$comment['ip']}) ";
		$html .= '的原贴：</span><span class="color_red">'.$comment['floornum'].'</span></div>';	
	...
	$_SGLOBAL['db']->query('UPDATE '.tname('spaceitems').' SET lastpost='.$_SGLOBAL['timestamp'].', replynum=replynum+1 WHERE itemid=\''.$itemid.'\'');
...
$channel = empty($item['type']) ? $type : $item['type'];
