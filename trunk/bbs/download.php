...
// only pages with NOVA_RUN defined can be run
	define("NOVA_RUN", 1);
...
$nova_domain 	= str_replace('/download.php', '', $my_address); 	// returns http://myforum.com/forum style address
...
// Can this member download this file?
	...
	if ($can_download_attachment=='1'){
		...
		header("Location: $nova_domain/uploads/attachments/$filename");
		exit;
		...
		header("HTTP/1.0 200 OK");
		header("Location: $nova_domain/index.php?page=error&error=23");
		exit;
		mysql_free_result($sql_result);
		mysql_close();
