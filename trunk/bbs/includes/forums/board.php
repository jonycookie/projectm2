...
die();
	...
	// check it the forum exists
		...
		if ($does_exist == '0'){
			nova_redirect("index.php?page=error&error=25","error/25");
		}
	...
	// otherwise, carry on displaying it
			...
			$query211 = "select NAME, PARENT, FORUM_RULES from {$db_prefix}categories WHERE ID='$forum_id'" ;
		...
		// list sub-forums...
			...
			if ($number_of_kids!='0'){                                  
				...
				if ($can_view_parent!='0'){
					...
					$query21 = "select ID, NAME, DESCRIPTION, READ_ONLY from {$db_prefix}categories WHERE PARENT='$forum' ORDER BY FORUM_ORDER, ID asc" ;
						...
						// PERMISSIONS!!! Can they view this forum???
							...
							if ($can_view_forum!='0'){
								...
								if ($read_only=='0'){
									...
									if (isset($nova_name)){			$unread_posts="0";
