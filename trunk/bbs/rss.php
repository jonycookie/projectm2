...
/*
	...
	$Id: rss.php 11555 2007-12-10 08:58:19Z tiger $
*/
	...
	foreach($_DCACHE['forums'] as $fid => $forum) {
		if($forum['type'] != 'group') {
			...
			while($thread = $db->fetch_array($query)) {
				...
				$thread['description'] = $thread['readperm'] > 0 || $thread['price'] > 0 || $thread['status'] ? '' : addslashes(cutstr(dhtmlspecialchars(preg_replace("/(\[.+\])/s", '', strip_tags(nl2br($thread['message'])))), 255));
