...
function novaquote(id){
	saved = new Array();
	clean = new Array();
	add   = 1;
	if ( tmp = nova_cookie('mqpid') )	{
		saved = tmp.split(",");
	}
	for(
	...
	nova_setcookie( 'mqpid', clean.join(','));
	return false;
	}
	
function nova_setcookie( name, value){
	document.cookie = name + "=" + value + "; path=/";
}

function nova_delcookie() {
