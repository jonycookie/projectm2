...
<?php }elseif ($template_hook=="3"){ ?>
					...
					<div style="width: 20%; float: left;">
						<a class="forum-index-link-to-topic" href="<?php echo nova_link("index.php?page=messages&act=new&id=$reported_by","messages/new/$reported_by"); ?>" title="<?php echo $lang_admin['report_pm']; ?>"><span class="warn-remove"><?php echo "$name"; ?></span></a>
					</div>
					<div style="width: 50%; float: left;">
						<?php echo "$content"; ?>
					</div>
					<div style="width: 15%; float: left; text-align: center;">
						<a href="<?php echo nova_link("index.php?page=admin&act=report&id=$id","admin/report/$id"); ?>"><img src="<?php echo "$plus_img"; ?>" alt="tick" title="<?php echo $lang_admin['report_action_taken']; ?>" /></a>
					</div>
