...
<?php $pages=(ceil($number_of_members/$list_posts)); if (!isset($_GET['limit'])){$_GET['limit']="1";} $start="1"; for($i = "1"; $i <= $pages; $i++) { $i_limit=$i; if ($i > 1){ $start=($start+1); } if ($_GET['limit']==$start){ ?>
		<a class="page-active" href="<?php echo nova_link("index.php?page=list&group=$group&limit=$i_limit", "list/group/$group/$i_limit"); ?>"><?php echo "$i"; ?></a>
<?php } else{ ?>
...
<?php if ($_GET['limit'] > "2" && $i_limit < $_GET['limit'] - "1"){ ?>
		<a class="page" href="<?php echo nova_link("index.php?page=list&group=$group&limit=1", "list/group/$group/1"); ?>">&lt;&lt;</a>
<?php } ?>
		<a class="page" href="<?php echo nova_link("index.php?page=list&group=$group&limit=$i_limit", "list/group/$group/$i_limit"); ?>"><?php echo "$i"; ?></a>
<?php }}} ?>
<?php if ($pages - $_GET['limit'] >= "3"){ ?>
		<a class="page" href="<?php echo nova_link("index.php?page=list&group=$group&limit=$pages_end", "list/group/$group/$pages_end"); ?>">&gt;&gt;</a>
<?php } ?>
		</div>
<?php } ?>
...
<?php if ($id=='0'){ ?>
					<img  src="<?php echo "$nova_domain"; ?>/<?php echo "$flag_path"; ?>/0.png" alt="" />
...
<?php }else{ ?>
					<!-- viewing topic -->
					<?php echo $lang['list_viewing_topic']; ?> <a class="forum-index-link-to-topic" href="<?php echo nova_link("index.php?topic=$location_topic", "topic/$topic_title-$location_topic"); ?>"><?php echo "$title"; ?></a>
<?php }} elseif($template_hook=='7'){ ?>
					<!-- viewing forum -->
					<?php echo $lang['list_viewing_forum']; ?> <a class="forum-index-link-to-topic" href="<?php echo nova_link("index.php?forum=$location_forum", "forum/$forum_title-$location_forum"); ?>"><?php echo "$forum_name"; ?></a>
...
<?php } elseif($template_hook=='12'){ ?>
		...
		<table class="forum-index" cellpadding="0" cellspacing="0">
			<tr>
				...
				<td class="forum-index-td-topics-sub" style="width: 15%; padding-left: 5px;">
					<?php echo $lang['list_time']; ?>
				</td>
...
<?php if ($pages <= '1'){}else{ ?>
				...
				<?php echo $lang['board_page']; ?>
<?php $pages=(ceil($number_of_members/$list_posts)); if (!isset($_GET['limit'])){$_GET['limit']="1";} $start="1"; for($i = "1"; $i <= $pages; $i++) { $i_limit=$i; if ($i > 1){ $start=($start+1); } if ($_GET['limit']==$start){ ?>
				<a class="page-active" href="<?php echo nova_link("index.php?page=list&limit=$i_limit", "online/$i_limit"); ?>"><?php echo "$i"; ?></a>
<?php } else{ ?>
<?php if ($_GET['limit'] >="3" && $i_limit < $_GET['limit'] - "2" OR $_GET['limit'] >="3" && $i_limit > $_GET['limit'] + "2"){}elseif($_GET['limit'] >="3" && $i_limit > $_GET['limit'] + "2"){}elseif($_GET['limit'] <= "2" && $i_limit > "4" ){}else{  ?>
<?php if ($_GET['limit'] > "2" && $i_limit < $_GET['limit'] - "1"){ ?>
				<a class="page" href="<?php echo nova_link("index.php?page=list&limit=1", "online/1"); ?>">&lt;&lt;</a>
<?php } ?>
				<a class="page" href="<?php echo nova_link("index.php?page=list&limit=$i_limit", "online/$i_limit"); ?>"><?php echo "$i"; ?></a>
<?php }}} ?>
<?php if ($pages - $_GET['limit'] >= "3"){ ?>
				<a class="page" href="<?php echo nova_link("index.php?page=list&limit=$pages_end", "online/$pages_end"); ?>">&gt;&gt;</a>
<?php } ?>
				</div>
<?php } ?>
...
<?php if ($sef_urls=='1'){ ?>
					<select name="forum" onchange="location.href='<?php echo "$nova_domain"; ?>/list/group/'+escape(this.options[this.selectedIndex].value)">
<?php }else{ ?>
					<select name="forum" onchange="location.href='<?php echo "$nova_domain"; ?>/index.php?page=list&group='+(this.options[this.selectedIndex].value)">
<?php } ?>
...
<?php if ($pages <= '1'){}else{ ?>
			...
			<?php echo $lang['board_page']; ?>
<?php $pages=(ceil($number_of_members/$list_posts)); if (!isset($_GET['limit'])){$_GET['limit']="1";} $start="1"; for($i = "1"; $i <= $pages; $i++) { $i_limit=$i; if ($i > 1){ $start=($start+1); } if ($_GET['limit']==$start){ ?>
			<a class="page-active" href="<?php echo nova_link("index.php?page=list&list=members&limit=$i_limit", "list/members/$i_limit"); ?>"><?php echo "$i"; ?></a>
<?php } else{ ?>
<?php if ($_GET['limit'] >="3" && $i_limit < $_GET['limit'] - "2" OR $_GET['limit'] >="3" && $i_limit > $_GET['limit'] + "2"){}elseif($_GET['limit'] >="3" && $i_limit > $_GET['limit'] + "2"){}elseif($_GET['limit'] <= "2" && $i_limit > "4" ){}else{  ?>
<?php if ($_GET['limit'] > "2" && $i_limit < $_GET['limit'] - "1"){ ?>
			<a class="page" href="<?php echo nova_link("index.php?page=list&list=members&limit=1", "list/members/1"); ?>">&lt;&lt;</a>
<?php } ?>
			<a class="page" href="<?php echo nova_link("index.php?page=list&list=members&limit=$i_limit", "list/members/$i_limit"); ?>"><?php echo "$i"; ?></a>
<?php }}} ?>
<?php if ($pages - $_GET['limit'] >= "3"){ ?>
			<a class="page" href="<?php echo nova_link("index.php?page=list&list=members&limit=$pages_end", "list/members/$pages_end"); ?>">&gt;&gt;</a>
<?php } ?>
			</div>
<?php } ?>	
