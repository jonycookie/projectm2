...
class LxBlog6 extends Blog{
	...
	function tags($num,$order='',$cid=0){
		...
		$rs = $this->mysql->query("SELECT tagid,tagname,$order AS value FROM {$this->config['dbpre']}btags ORDER BY $order DESC LIMIT $num");
	...
	function getBlogs($start,$num,$cid=0,$type='blog',$order=''){
		...
		!in_array($type,array('blog')) && $type = 'blog';
		$order = addslashes($order);
	...
	function malbums($num,$order='',$cid=0){
		$num = (int)$num;
		!$num && $num=10;
		$cid = (int)$cid;
		$order = $order ? $order : 'postdate';
		$where = '';
		if($cid) {
			$where = "WHERE cid='$cid'";
		}
		$malbums = array();
		$rs = $this->mysql->query("SELECT * FROM {$this->config['dbpre']}malbums $where ORDER BY $order DESC LIMIT 0,$num");
		while($rt = $this->mysql->fetch_array($rs)) {
			$rt['url'] = $this->config['url']."/blog.php?do=showone&uid=$rt[uid]&type=music&maid=$rt[maid]";
			$rt['title'] = $rt['subject'];
			$malbums[]=$rt;
		}
		return $malbums;
	...
	function music($num,$order='',$cid=0){
		...
		!$num && $num=10;
		$cid = (int)$cid;
		$order = $order ? $order : 'posttime';
		$music = array();
		$rs = $this->mysql->query("SELECT * FROM {$this->config['dbpre']}music ORDER BY $order DESC LIMIT 0,$num");
		while($rt = $this->mysql->fetch_array($rs)) {
			$music[]=$rt;
		}
		return $music;
	 ...
	 * @return array
	 */
	function albums($num,$order='',$cid=0){
		$num = (int)$num;
		!$num && $num=10;
		$cid = (int)$cid;
		$order = $order ? $order : 'postdate';
		$order = 'a.'.$order;
		$where = "WHERE p.ifhpage=1";
		if($cid) {
			$where .= "AND cid='$cid'";
		}
		$albums = array();
		$rs = $this->mysql->query("SELECT a.*,p.* FROM {$this->config['dbpre']}albums a LEFT JOIN {$this->config['dbpre']}photo p USING(aid) $where ORDER BY $order DESC LIMIT 0,$num");
		while($rt = $this->mysql->fetch_array($rs)) {
			$rt['url'] = $this->config['url']."/blog.php?do=list&uid=$rt[uid]&type=photos&job=view&aid=$rt[aid]";
			if($rt['ifhpage']) {
				$rt['imgurl'] = $this->config['attachurl'].$rt['attachurl'];
			}
			$rt['title'] = $rt['subject'];
			$albums[]=$rt;
		}
	...
	function photo($num,$order='',$cid=0){
		...
		!$num && $num=10;
		$cid = (int)$cid;
		$order = $order ? $order : 'uploadtime';
		$where = '';
		if($cid) {
			$where = "WHERE cid='$cid'";
		}
		$photo = array();
		$rs = $this->mysql->query("SELECT * FROM {$this->config['dbpre']}photo $where ORDER BY $order DESC LIMIT 0,$num");
		while($rt = $this->mysql->fetch_array($rs)) {
			$rt['attachurl'] = $this->config['attachurl'].$rt['attachurl'];
			$photo[]=$rt;
		}
		return $photo;
	}
	...
	function getBlog($cid,$start,$num,$order='') {
		...
		$type != 'blog' && Showmsg('cate_blogerror');
