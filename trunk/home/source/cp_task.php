...
/*
	...
	$Id: cp_task.php 10322 2008-11-28 09:44:18Z liguode $
*/
...
if($taskid) {
	...
	if($_GET['view'] == 'member') {
		...
		if($count) {
			$query = $_SGLOBAL['db']->query("SELECT s.*, main.dateline
		...
		if($usertask = $_SGLOBAL['db']->fetch_array($query)) {
			if($task['maxnum'] && $task['maxnum']<=$task['num']) {
				$task['done'] = 1;//最大次数
			} elseif($task['nexttime'] && $_SGLOBAL['timestamp']-$usertask['dateline']>=$task['nexttime']) {
				$task['done'] = 0;
			} else {
				$task['done'] = 1;
			}
			$task['result'] = $usertask['result'];
			...
			if($task['done']) {
				...
				$fs = array(
					...
					'title_data' => array(
							...
							'credit' => '<span style="color:red;">'.$task['credit']."</span>"
						),
				);
	...
	if($_GET['view'] == 'done') {
		...
		while ($value = $_SGLOBAL['db']->fetch_array($query)) {
			if((empty($value['maxnum']) || $value['maxnum']>$value['num']) &&
				...
				$all_num++;
				if(empty($usertasks[$value['taskid']]) || ($value['nexttime'] && $_SGLOBAL['timestamp']-$usertasks[$value['taskid']]['dateline']>=$value['nexttime'])) {
					$todo_num++;
					$value['image'] = empty($value['image'])?'image/task.gif':$value['image'];
					$value['done'] = 0;
					$tasklist[$value['taskid']] = $value;
