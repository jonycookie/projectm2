...
/*
	...
	$Id: function_admincp.php 10789 2008-12-23 02:39:41Z liguode $
*/
...
function getstatistics() {
	...
	$statistics = array(
		...
		'feednum1' => $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT COUNT(*) FROM ".tname('feed')." WHERE appid='".UC_APPID."'"), 0),
		'mtagnum' => getcount('mtag', array()),
		...
		'spacenum' => $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT COUNT(*) FROM ".tname('space')." WHERE updatetime>0"), 0),
		'spacenum1' => $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT COUNT(*) FROM ".tname('space')." WHERE friendnum<10 AND friendnum>0"), 0),
		'spacenum2' => $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT COUNT(*) FROM ".tname('space')." WHERE friendnum>=10 AND friendnum<100"), 0),
		'spacenum3' => $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT COUNT(*) FROM ".tname('space')." WHERE friendnum>=100"), 0),
		'spacenum4' => $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT COUNT(*) FROM ".tname('space')." WHERE namestatus=1"), 0),
		'spacenum5' => $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT COUNT(*) FROM ".tname('space')." WHERE avatar=1"), 0),
		'spacenum6' => $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT COUNT(*) FROM ".tname('space')." WHERE credit>=20"), 0),
		'spacenum7' => $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT COUNT(*) FROM ".tname('space')." WHERE dateline<lastlogin"), 0),
		'spacenum8' => $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT COUNT(*) FROM ".tname('space')." WHERE dateline<updatetime"), 0)
	);
