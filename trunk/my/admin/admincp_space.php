...
/*
	...
	$Id: admincp_space.php 12789 2009-07-21 01:16:31Z zhengqingpeng $
*/
...
if($uid) {
	...
	if($_SCONFIG['videophoto'] && $member['videopic'] && $member['videostatus']) {
		...
		$member['videopic'] = getvideopic($member['videopic']);
	} else {
		$member['videopic'] = '';
...
if(submitcheck('usergroupsubmit')) {
	...
	if($managespaceinfo) {
		...
		if(empty($_POST['videostatus'])) $member['videopic'] = '';
	...
	if($touids && $mids) {
		...
		while($value = $_SGLOBAL['db']->fetch_array($query)) {
			$names[$value['uid']] = $value['username'];
		}
...
$randkeys = array(array('sstrtotime','dateline'), array('sstrtotime','updatetime'), array('sstrtotime','lastpost'), array('sstrtotime','lastlogin'), array('intval','credit'));
$likekeys = array('name');
...
//排序
$orders = getorders(array('dateline', 'updatetime', 'friendnum', 'credit', 'viewnum'), 'uid', $pre);
$ordersql = $orders['sql'];
