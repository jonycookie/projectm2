...
/*
	...
	$Id: admincp.php 12790 2009-07-21 01:17:06Z zhengqingpeng $
*/
...
$acs = array(
	...
	array('cache', 'network', 'profilefield', 'eventclass', 'click', 'task', 'censor', 'stat', 'block', 'cron', 'app'),
...
for($i=0; $i<3; $i++) {
	foreach ($acs[$i] as $value) {
		...
		} elseif($value == 'space' && ($megroup['managename'] || $megroup['managespacegroup'] || $megroup['managespaceinfo'] || $megroup['managespacecredit'] || $megroup['managespacenote'] || $megroup['managedelspace'])) {
			$needlogin = 1;
			$menus[$i]['space'] = 1;
