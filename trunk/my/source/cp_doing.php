...
/*
	...
	$Id: cp_doing.php 12788 2009-07-21 01:05:22Z liguode $
*/
...
if(submitcheck('addsubmit')) {
	...
	if($add_doing) {
		if(empty($space['doingnum'])) {//第一次
			$space['doingnum'] = getcount('doing', array('uid'=>$space['uid']));
		}
		$setarr['doingnum'] = "doingnum='".($space['doingnum'] + 1)."'";
	}
...
if($_GET['op'] == 'delete') {
	...
	if(empty($_GET['close'])) {
		...
		while ($value = $_SGLOBAL['db']->fetch_array($query)) {
			...
			$count++;
			if($value['authorid'] = $space['uid']) $height = $value['id'];
		}
	}
	...
	if($count) {
		...
		foreach ($values as $key => $vid) {
			...
			if($one['id'] == $height && $one['uid'] == $space['uid']) {
