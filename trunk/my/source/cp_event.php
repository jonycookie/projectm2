...
if(submitcheck('eventsubmit')) {
	...
	if($eventid){// 修改已有活动
		...
		if(empty($space['eventnum'])) {
			...
			$eventnumsql = "eventnum=".($space['eventnum']+1);
		} else {
...
if($op == 'invite') {
	...
	if(submitcheck('invitesubmit')){
		...
		showmessage("do_success", "cp.php?ac=event&op=invite&id=$eventid&group=$group", 2);
	}
	...
	$week = date("w",$daystart);//本月第一天是周几: 0-6	
	$dayscount = date("t",$daystart);//本月天数
	...
	//本月活动
	$events = array();
	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname("event")." WHERE starttime < $dayend AND endtime > $daystart ORDER BY eventid DESC LIMIT 200");//最多只取200
	while($value=$_SGLOBAL['db']->fetch_array($query)) {
		...
		for($i=$start; $i<=$end; $i++) {
			if($days[$i]['count'] < 20) {//最多只显示20个活动/每天
				$days[$i]['events'][] = $value;
				$days[$i]['count'] += 1;
