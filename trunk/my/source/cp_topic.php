...
if($_GET['op'] == 'delete') {
	...
	if(submitcheck('ignoresubmit')) {
		...
		if($tablename && $id) {
			updatetable($tablename, array('topicid'=>0), array($idtype=>$id, 'topicid'=>$topicid));
			$_SGLOBAL['db']->query("DELETE FROM ".tname('topicuser')." WHERE uid='$space[uid]' AND topicid='$topicid'");
		}
		showmessage('do_success', $_POST['refer'], 0);
	}
	...
	if(submitcheck('joinsubmit')) {
		...
		//参与用户
		if($_POST['newtopicid']) {
			topic_join($_POST['newtopicid']);
		} else {
			$_SGLOBAL['db']->query("DELETE FROM ".tname('topicuser')." WHERE uid='$space[uid]' AND topicid='$item[topicid]'");
		}
