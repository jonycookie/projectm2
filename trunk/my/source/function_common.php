...
/*
	...
	$Id: function_common.php 13003 2009-08-05 06:46:06Z liguode $
*/
...
function avatar($uid, $size='small', $returnsrc = FALSE) {
	...
	return $returnsrc ? UC_API.'/data/avatar/'.$avatarfile : '<img src="'.UC_API.'/data/avatar/'.$avatarfile.'" onerror="this.onerror=null;this.src=\''.UC_API.'/images/noavatar_'.$size.'.gif\'" alt="'.$_SN[$uid].'">';
}
...
function topic_get($topicid) {
	...
	if($topicid) {
		...
		if($topic = $_SGLOBAL['db']->fetch_array($query)) {
			$topic['pic'] = $topic['pic']?pic_get($topic['pic'], $topic['thumb'], $topic['remote']):'';
