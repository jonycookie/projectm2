...
function feed_publish($id, $idtype, $add=0) {
	...
	switch ($idtype) {
		...
		case 'albumid':
			$key = 1;
			if($id > 0) {
				$query = $_SGLOBAL['db']->query("SELECT a.albumname, a.picnum, a.friend, a.target_ids, p.* FROM ".tname('pic')." p
		...
		case 'picid':
			...
			$query = $_SGLOBAL['db']->query("SELECT a.friend, a.target_ids, s.username, p.* FROM ".tname('pic')." p
