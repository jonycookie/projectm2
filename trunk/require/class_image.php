...
class image{
    ...
    var $img_display_quality = 90;// 图片显示质量,默认为90
     ...
     * 构造函数
     */
    function image($src_img){
        $this->__construct($src_img);
    }

	function __construct($src_img){ //PHP5 操作类型
		$this->src_img = $src_img;
		$this->mask_font_color = "#000000";
        //$this->font = D_P."require/encode/STXINWEI.TTF";
        $this->font_size = 12;
		$this->setSrcImg($this->src_img);
	}
    ...
    function setMaskWord($word){
        $this->mask_word .= $this->Convert($word);
    }
     ...
     */
    function setMaskFontColor($color="#000000"){
    ...
    function setMaskFont($font=2){
        if(!is_numeric($font) && !file_exists(D_P."require/encode/".$font)){
            $this->watermsg("字体文件不存在");
        }
        $this->font = $font;
		$this->font = D_P."require/encode/".$font;
    }
    ...
    function createImg($a, $b=null){
        ...
        if(2 == $num){
            ...
            $h = (int)$b;
            if(0 == $w){
                //$this->watermsg("目标宽度不能为0");
				$w = $this->src_w;
            }
            if(0 == $h){
                //$this->watermsg("目标高度不能为0");
				$h = $this->src_h;
            }
    ...
    function _countMaskPos(){
		$insertfile_width  = $this->font_w?$this->font_w:$this->mask_w;
		$insertfile_height = $this->font_h?$this->font_h:$this->mask_h;
        if($this->_isFull()){
            switch($this->mask_position){
				case 0:
                    // 中间
                    $this->mask_pos_x = ($this->src_w + $this->img_border_size)/2 -$insertfile_width/2;
                    $this->mask_pos_y = ($this->src_h + $this->img_border_size)/2 -$insertfile_height/2;
                    break;

                case 1:
                    // 左上
                    $this->mask_pos_x = $this->mask_offset_x + $this->img_border_size;
                    $this->mask_pos_y = $this->mask_offset_y + $this->img_border_size+$insertfile_height;
                    break;
    ...
    function _setNewImgSize($img_w, $img_h=null){
        ...
        if(2 == $num){
            ...
            if($fill_w < 0 || $fill_h < 0){
                $this->watermsg("图片边框过大，已超过了图片的宽度");
            }
    ...
    function _checkMaskValid(){
        if(    $this->mask_w + $this->mask_offset_x > $this->src_w
            ...
            $this->watermsg("水印图片尺寸大于原图，请缩小水印图");
        }
    }
	 ...
	 * @param string $str 字符
     */
	function watermsg($str){
	P_unlink($this->src_img);
	Showmsg($str);
	}
}
?>