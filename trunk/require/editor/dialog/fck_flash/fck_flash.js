...
window.onload = function()
{
	...
	if ( FCKConfig.FlashUpload )
		GetE('frmUpload').action = FCKConfig.BaseName +"?adminjob=edit&action=fckupload" ;
...
function BrowseServer(inputname)
{
	...
	if(sSuffix=='ie'){
		showModalDialog(FCKConfig.BaseName+"?adminjob=edit&action=addmedia&type=flash&inputtype=editor&inputname="+inputname+"&time="+timestamp,window,'dialogWidth=600px;dialogHeight=400px;help:no;status:no;');
	}else{
		window.open(FCKConfig.BaseName+"?adminjob=edit&action=addmedia&type=flash&inputtype=editor&inputname="+inputname+"&time="+timestamp,'','');
	}
}
