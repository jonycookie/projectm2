...
/*
	...
	$Id: admin_prefields.php 13382 2009-10-09 07:06:41Z zhaofei $
*/
...
if(!empty($typearr) && is_array($typearr)) {
	foreach ($typearr as $typeid => $typename) {
		...
		echo '<td'.$class.'><a href="'.$theurl.'&type='.$typeid.'">'.$typename.'</a></td>';
	}
}
echo '</tr>
</table>
...
if(is_array($thevalue) && $thevalue) {
	...
	echo label(array('type'=>'select', 'alang'=>'prefield_title_field', 'name'=>'field', 'options'=>$fieldarr[$thevalue['type']], 'width'=>'30%', 'value'=>$field));
...
if(is_array($listarr) && $listarr) {
	...
	foreach ($listarr as $listvalue) {
		...
		$listvalue['field'] = $fieldarr[$listvalue['type']][$listvalue['field']];
		$listvalue['type'] = $typearr[$listvalue['type']];
