...
/*
	...
	$Id: uc.php 11538 2009-03-09 08:15:35Z zhaofei $
*/
...
if(defined('IN_UC')) {
	...
	global $_SGLOBAL, $_SCONFIG, $_SC, $_SCOOKIE;
...
class uc_note {
	...
	function deleteuser($get, $post) {
		...
		//获得用户
		$uids = $get['ids'];
		$query = $_SGLOBAL['db']->query("SELECT uid FROM ".tname('members')." WHERE uid IN ($uids)");
	...
	function renameuser($get, $post) {
		...
		$_SGLOBAL['db']->query("UPDATE ".tname('members')." SET username='$new_username' WHERE username='$old_username'");
		$_SGLOBAL['db']->query("UPDATE ".tname('robotitems')." SET username='$new_username' WHERE username='$old_username'");
		$_SGLOBAL['db']->query("UPDATE ".tname('spaceitems')." SET username='$new_username' WHERE username='$old_username'");
		$_SGLOBAL['db']->query("UPDATE ".tname('tags')." SET username='$new_username' WHERE username='$old_username'");
