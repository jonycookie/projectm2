...
<!--{if $op == 'list'}-->
	...
	<div class="global_module margin_bot10 bg_fff userpanel">
		...
		<!--{else}-->
			<div class="user_no_body">没有符合条件的信息</div>
		<!--{/if}-->
	</div>
	</form>
	...
	<div class="col2" >
		...
		<!--{if $do == 'me'}-->
			<!--{block name="postitem" parameter="type/$type/uid/$_SGLOBAL[supe_uid]/dateline/604800/order/i.click_35 DESC,i.dateline DESC/limit/0,10/cachetime/900/subjectlen/32/cachename/mynews"}-->
		<!--{elseif $do=='pass'}-->
		...
		<!--{else}-->
			<!--{block name="postitem" parameter="type/$type/dateline/604800/order/i.click_35 DESC,i.dateline DESC/limit/0,10/cachetime/900/subjectlen/40/cachename/mynews"}-->
		<!--{/if}-->
		<div class="global_module bg_fff margin_bot10">
			...
			<li><span class="box_r">$value[click_9] $value[click_35]</span><a href="$value[url]" title="$value[subjectall]">$value[subject]</a></li>
			<!--{/loop}-->
			</ul>
		</div>
	
		<!--{if $do == 'me'}-->
			<!--{block name="postitem" parameter="type/$type/uid/$_SGLOBAL[supe_uid]/order/i.click_36 DESC,i.dateline DESC/limit/0,10/cachetime/900/subjectlen/32/cachename/mynews2"}-->
		<!--{elseif $do=='pass'}-->
		...
		<!--{else}-->
			<!--{block name="postitem" parameter="type/$type/order/i.click_36 DESC,i.dateline DESC/limit/0,10/cachetime/900/subjectlen/40/cachename/mynews2"}-->
		<!--{/if}-->
		<div class="global_module bg_fff">
			<div class="global_module2_caption"><h3>一周被踩最多的文章</h3></div>
			<ul class="global_tx_list2">
			<!--{loop $_SBLOCK['mynews2'] $value}-->
			<li><span class="box_r">$value[click_10] $value[click_36]</span><a href="$value[url]" title="$value[subjectall]">$value[subject]</a></li>
			<!--{/loop}-->
			</ul>
		</div>
	</div><!--col2-->
