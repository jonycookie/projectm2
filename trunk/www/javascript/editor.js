 ...
 */
(function($){
$.fn.editor=function(bInit,options)
{
	return this.each(function(){
		...
		if(bInit)//初始化
		{
			if(!this.editor)
			{
				...
				if(editor.init())this.editor=editor;			
				else editor=null;	
			}
		}
		else//卸载
...
var htmlPastetext='<div>使用键盘快捷键(Ctrl+V)把内容粘贴到方框里，按 确定</div><div><textarea id="EdtPastetextValue" wrap="soft" spellcheck="false" style="width:300px;height:100px;" /></div><div style="text-align:right;"><input type="button" id="EdtSave" value="确定" /></div>';
var htmlLink='<div>链接地址: <input type="text" id="EdtLinkHref" value="http://" class="text" /></div><div>打开方式: <select id="EdtLinkTarget"><option selected="selected" value="">默认</option><option value="_blank">新窗口</option><option value="_self">当前窗口</option><option value="_parent">父窗口</option></select></div><div style="text-align:right;"><input type="button" id="EdtSave" value="确定" /></div>';
var htmlImg='<div>图片地址：<input type="text" id="EdtImgSrc" value="http://" class="text" /></div><div>替换文本：<input type="text" id="EdtImgAlt" /></div><div>对齐方式：<select id="EdtImgAlign"><option selected="selected" value="">默认</option><option value="left">左对齐</option><option value="right">右对齐</option><option value="top">顶端</option><option value="middle">居中</option><option value="baseline">基线</option><option value="bottom">底边</option></select></div><div>宽度高度：<input type="text" id="EdtImgWidth" style="width:40px;" /> x <input type="text" id="EdtImgHeight" style="width:40px;" /></div><div>边框大小：<input type="text" id="EdtImgBorder" style="width:40px;" /></div><div>水平间距：<input type="text" id="EdtImgVspace" style="width:40px;" /> 垂直间距：<input type="text" id="EdtImgHspace" style="width:40px;" /></div><div style="text-align:right;"><input type="button" id="EdtSave" value="确定" /></div>';
var htmlFlash='<div>动画地址：<input type="text" id="EdtFlashSrc" value="http://" class="text" /></div><div>宽度高度：<input type="text" id="EdtFlashWidth" style="width:40px;" value="480" /> x <input type="text" id="EdtFlashHeight" style="width:40px;" value="400" /></div><div style="text-align:right;"><input type="button" id="EdtSave" value="确定" /></div>';
var htmlMedia='<div>视频地址：<input type="text" id="EdtMediaSrc" value="http://" class="text" /></div><div>宽度高度：<input type="text" id="EdtMediaWidth" style="width:40px;" value="480" /> x <input type="text" id="EdtMediaHeight" style="width:40px;" value="400" /></div><div style="text-align:right;"><input type="button" id="EdtSave" value="确定" /></div>';
var htmlTable='<div>行数列数：<input type="text" id="EdtTableRows" style="width:40px;" value="3" /> x <input type="text" id="EdtTableColumns" style="width:40px;" value="2" /></div><div>标题单元：<select id="EdtTableHeaders"><option selected="selected" value="">无</option><option value="row">第一行</option><option value="col">第一列</option><option value="both">第一行和第一列</option></select></div><div>宽度高度：<input type="text" id="EdtTableWidth" style="width:40px;" value="200" /> x <input type="text" id="EdtTableHeight" style="width:40px;" value="" /></div><div>边框大小：<input type="text" id="EdtTableBorder" style="width:40px;" value="1" /></div><div>间距边距：<input type="text" id="EdtTableCellSpacing" style="width:40px;" value="1" /> <input type="text" id="EdtTableCellPadding" style="width:40px;" value="1" /></div><div>对齐方式：<select id="EdtTableAlign"><option selected="selected" value="">默认</option><option value="left">左对齐</option><option value="center">居中</option><option value="right">右对齐</option></select></div><div>表格标题：<input type="text" id="EdtTableCaption" /></div><div style="text-align:right;"><input type="button" id="EdtSave" value="确定" /></div>';
var htmlAbout='<div style="width:200px;word-wrap:break-word;word-break:break-all;"><p><span style="font-size:20px;color:#1997DF;">Editor</span><br />版本：v0.9.8 build 20090822</p><p>Editor是一个基于jQuery开发的跨平台开源迷你XHTML编辑器组件。</p><p><a href="http://editor.googlecode.com/" target="_blank">http://editor.googlecode.com/</a></p></div><div style="text-align:right;"><input type="button" id="EdtSave" value="确定" /></div>';
...
$.editor=function(textarea,options)
{
	...
	{
		//加载样式表
		if($('#'+idCSS).size()==0)$('head').append('<link id="'+idCSS+'" rel="stylesheet" type="text/css" href="'+jsURL+'skin/'+_this.settings.skin+'/ui.css" />');
		...
		_jText.after($('<span id="'+idContainer+'" class="Edt_'+_this.settings.skin+'" style="display:none"><table cellspacing="0" cellpadding="0" class="EdtLayout" style="width:'+cw+';height:'+editHeight+'px;"><tbody><tr><td id="'+idTools+'" class="EdtTool" style="height:'+toolbarHeight+'px"></td></tr><tr><td id="'+idIframeArea+'" class="EdtIframeArea" style="height:'+(editHeight-toolbarHeight)+'px"><iframe frameborder="0" id="'+idIframe+'" src="javascript:;" style="width:100%;" /></td></tr></tbody></table></span>'));
		_win=$('#'+idIframe)[0].contentWindow;
		_jWin=$(_win);
		try{
			_doc = _win.document;_jDoc=$(_doc);
			_doc.open();
			_doc.write('<html><head><base href="'+baseURL+'"/><meta content="text/html; charset=UTF-8" http-equiv="Content-Type"/><link rel="stylesheet" href="'+jsURL+'skin/'+_this.settings.skin+'/iframe.css"/>');
		...
		{
			tool=arrTools[n];
			if(n=='GStart')sToolHtml+='<td><span class="EdtGStart"/></td>';
			else if(n=='GEnd')sToolHtml+='<td><span class="EdtGEnd"/></td>';
			else if(n=='Separator')sToolHtml+='<td><span class="EdtSeparator"/></td>';
			else
			{
				var cn;
				if(tool.c)cn=tool.c;
				else cn='EdtIcon EdtBtn'+n;
				sToolHtml+='<td><a href="javascript:;" title="'+tool.t+'" name="'+n+'" class="EdtButton EdtEnabled"><span class="'+cn+'"/></a></td>';
				if(tool.s)_this.addShortCut(tool.s,n);
			}
		});
		sToolHtml+='</tr></tbody></table>';
		_jTools=$('#'+idTools).append(sToolHtml);
		_jTools.find('.EdtButton').click(function(event)
		{
			_this.hidePanel();
			_this.focus();
			ev=event;
			var aButton=$(this);
			if(aButton.is('.EdtEnabled'))_this.exec(aButton.attr('name'));
			ev.stopPropagation();
		}).mousedown(function(){return false;});
		//初始化面板
		_jPanel=$('#EdtPanel');
		_jCntLine=$('#EdtCntLine');
		if(_jPanel.size()==0)
		{
			_jPanel=$('<div id="EdtPanel"></div>').mousedown(function(ev){ev.stopPropagation()});
			_jCntLine=$('<div id="EdtCntLine"><img src="'+jsURL+'skin/'+_this.settings.skin+'/img/spacer.gif" /></div>');
	...
	this.processHTML=function(sHtml,mode)
	{
		...
		{//read
			if(_this.settings.keepValue)//恢复属性值
			{
				...
				sHtml = sHtml.replace(/<(\w+(?:\:\w+)?)(\s+[^>]*?_e_(src|href)\s*=\s*(['"]?)\s*([^'"\s]*)\s*\4[^>]*)>/ig,restoreValue);
			}
			if(isSafari)
			{
				sHtml = sHtml.replace(/("|;)\s*font-size\s*:\s*([a-z-]+)(;?)/ig,function(all,pre,sname,aft){
			...
			sHtml = sHtml.replace(/(\s+)(?:_e_|_moz_|_webkit_)[^=]+?\s*=\s*(["']?)[^"'\s>]*\2\s*/ig,'$1');
	...
	this.toggleReadonly=function(state)
	{
		...
		if(bReadonly)
		{
			...
			_jTools.find('[name=Preview]').toggleClass('EdtEnabled').toggleClass('EdtActive');
		}
		else
		{
			_jTools.find('[name=Preview]').toggleClass('EdtEnabled').toggleClass('EdtActive');
	...
	this.toggleSource=function(state)
	{
		...
		_jTools.find('[name=Source]').toggleClass('EdtEnabled').toggleClass('EdtActive');
		_jTools.find('.EdtButton').not('[name=Source],[name=Fullscreen],[name=About]').toggleClass('EdtEnabled');
		...
		_jTools.find('[name=Source]').toggleClass('EdtEnabled');
		setTimeout(_this.setOpts,300);
	}
	this.togglePreview=function(state)
	{
		if(bSource||bPreview===state)return;
		_jTools.find('[name=Preview]').toggleClass('EdtActive').toggleClass('EdtEnabled');
		_jTools.find('.EdtButton').not('[name=Preview],[name=Fullscreen],[name=About]').toggleClass('EdtEnabled');
		var jBody=$(_doc.body);
		if(!bPreview)
		{//转预览模式
			if(isIE)_doc.body.contentEditable='false';
		...
		bPreview=!bPreview;
		_jTools.find('[name=Preview]').toggleClass('EdtEnabled');
		setTimeout(_this.setOpts,300);
	}
	this.toggleFullscreen=function(state)
	{
		...
		if(bShowPanel)_this.hidePanel();
		var jArea=$('#'+idIframeArea),jLayout=$('#'+idContainer).find('.EdtLayout'),jContainer=$('#'+idContainer);
		if(bFullscreen)
		{//取消全屏
		...
		jContainer.toggleClass('Edt_Fullscreen');
		$('html').toggleClass('Edt_Fullfix');
		_jTools.find('[name=Fullscreen]').toggleClass('EdtActive');
		setTimeout(_this.setOpts,300);
	}
	...
	{
		var jMenu=$('<div class="EdtMenu"></div>'),jItem;
	...
	{
		var jColor=$('<div class="EdtColor"></div>'),jLine,jItem,c=0;
		jLine=$('<div></div>');
		$.each(itemColors,function(n,v)
		{
			c++;
			jItem=$('<a href="javascript:;" title="'+v+'" style="background:'+v+'"><img src="'+jsURL+'skin/'+_this.settings.skin+'/img/spacer.gif" /></a>').click(function(){_this.focus();callback(v);_this.hidePanel();}).mousedown(function(){return false;});
			jLine.append(jItem);
			if(c%10==0)
			{
				jColor.append(jLine);
	...
	this.showPastetext=function()
	{
		...
		var jValue=$('#EdtPastetextValue',jPastetext),jSave=$('#EdtSave',jPastetext);
	...
	{
		var jLink=$(htmlLink);
		var jParent=_this.getParent('a'),jHref=$('#EdtLinkHref',jLink),jTarget=$('#EdtLinkTarget',jLink),jSave=$('#EdtSave',jLink);
		if(jParent.size()==1)
		{
		...
		jSave.click(function(){
			...
			if(url!='')
			{
				if(jParent.size()==0)
				{
					_this._exec('createlink','#edt_tempurl');
					jParent=$('a[href$="#edt_tempurl"]',_doc);
				}
	...
	{
		var jImg=$(htmlImg);
		var jParent=_this.getParent('img'),jSrc=$('#EdtImgSrc',jImg),jAlt=$('#EdtImgAlt',jImg),jAlign=$('#EdtImgAlign',jImg),jWidth=$('#EdtImgWidth',jImg),jHeight=$('#EdtImgHeight',jImg),jBorder=$('#EdtImgBorder',jImg),jVspace=$('#EdtImgVspace',jImg),jHspace=$('#EdtImgHspace',jImg),jSave=$('#EdtSave',jImg);
		if(jParent.size()==1)
		{
		...
		jSave.click(function(){
			...
			if(url!='')
			{
				...
				if(jParent.size()==0)
				{
					_this.pasteHTML('<img src="#edt_tempurl" />',true);
					jParent=$('img[src$="#edt_tempurl"]',_doc);
				}
	...
	this.showFlash=function()
	{
		...
		var jParent=_this.getParent('embed[type="application/x-shockwave-flash"]'),jSrc=$('#EdtFlashSrc',jFlash),jWidth=$('#EdtFlashWidth',jFlash),jHeight=$('#EdtFlashHeight',jFlash),jSave=$('#EdtSave',jFlash);
		if(jParent.size()==1)
		{
			jSrc.val(_this.attr(jParent,'src'));
			jWidth.val(jParent.attr('width'));
		...
		jSave.click(function(){
			...
			if(sSrc!='')
			{
				if(jParent.size()==0)
				{
					_this.pasteHTML('<embed type="application/x-shockwave-flash" src="#edt_tempurl" wmode="opaque" quality="high" bgcolor="#ffffff" menu="false" play="true" loop="true" />');
					jParent=$('embed[src$="#edt_tempurl"]',_doc);
				}
	...
	this.showMeida=function()
	{
		...
		var jParent=_this.getParent('embed[type="application/x-mplayer2"]'),jSrc=$('#EdtMediaSrc',jMedia),jWidth=$('#EdtMediaWidth',jMedia),jHeight=$('#EdtMediaHeight',jMedia),jSave=$('#EdtSave',jMedia);
		if(jParent.size()==1)
		{
			jSrc.val(_this.attr(jParent,'src'));
			jWidth.val(jParent.attr('width'));
		...
		jSave.click(function(){
			...
			if(sSrc!='')
			{
				if(jParent.size()==0)
				{
					_this.pasteHTML('<embed type="application/x-mplayer2" src="#edt_tempurl" enablecontextmenu="false" autostart="false" />');
					jParent=$('embed[src$="#edt_tempurl"]',_doc);
				}
				_this.attr(jParent,'src',sSrc);
				var w=jWidth.val(),h=jHeight.val(),reg=/^[0-9]+$/;
	...
	this.ajaxUploadInit=function(jText,tourl,upext)
	{
		...
		{//系统默认ajax上传
			jUpload=$(s1+'<input type="file" class="file" size="11" name="upload" id="EdtUploadFile" /></span>');
			jText.after(jUpload);
			$('.btn',jUpload).before(jText);
			var jFile=$('#EdtUploadFile',jUpload);
	...
	this.ajaxUpload=function(fromfile,tourl,callback)
	{
		...
		var jIO=$('<iframe name="'+idIO+'" class="EdtUploadIO" />').appendTo('body');
		var jForm=$('<form action="'+tourl+'" target="'+idIO+'" method="post" enctype="multipart/form-data" class="EdtUploadForm"></form>').appendTo('body');
	...
	{
		var jOverlay=$('<div class="EdtOverlay"></div>').appendTo('body').mousedown(function(){return false;});
		w=w?w:_this.settings.modalWidth;h=h?h:_this.settings.modalHeight;
		var jModal=$('<div class="EdtModal" style="width:'+w+'px;height:'+h+'px;margin:-'+Math.ceil(h/2)+'px 0px 0px -'+Math.ceil(w/2)+'px;">'+(_this.settings.modalTitle?'<div class="EdtTitle"><span class="EdtClose" title="Close"></span>'+title+'</div>':'')+'<div class="EdtContent"></div></div>').appendTo('body').mousedown(function(){return false;});;
		$('.EdtClose',jModal).click(unloadme);
		function unloadme(){jModal.remove();jOverlay.remove();}
		var jIframe=$('<iframe frameborder="0" src="'+ifmurl+'" style="width:100%;" />');
		$('.EdtContent',jModal).append(jIframe);
	...
	{
		var jEmot=$('<div class="EdtEmot"></div>'),jLine,jItem,c=0,sEmotPath=jsURL+'emot/default/';
		jLine=$('<div></div>');
	...
	this.showTable=function()
	{
		...
		var jRows=$('#EdtTableRows',jTable),jColumns=$('#EdtTableColumns',jTable),jHeaders=$('#EdtTableHeaders',jTable),jWidth=$('#EdtTableWidth',jTable),jHeight=$('#EdtTableHeight',jTable),jBorder=$('#EdtTableBorder',jTable),jCellSpacing=$('#EdtTableCellSpacing',jTable),jCellPadding=$('#EdtTableCellPadding',jTable),jAlign=$('#EdtTableAlign',jTable),jCaption=$('#EdtTableCaption',jTable),jSave=$('#EdtSave',jTable);
		jSave.click(function(){
			_this.focus();
	...
	{
		var jDialog=$('<div class="EdtDialog"></div>');
	...
	this.showPanel=function(content)
	{
		...
		_jPanelButton.addClass('EdtActive');
	...
	this.hidePanel=function(){if(bShowPanel){_jPanelButton.removeClass('EdtActive');_jCntLine.hide();_jPanel.hide();bookmark=null;bShowPanel=false;}}
...
$(function(){
$('textarea.editor').editor(true);
$('textarea.editor-mini').editor(true,{tools:'mini'});
$('textarea.editor-simple').editor(true,{tools:'simple'});
});

})(jQuery);
/*
接口：
var editor=$('#elm1').editor(true,{tools:'full',skin:'default',showBlocktag:false,internalScript:true,internalStyle:true,width:300,height:200,loadCSS:'http://pirate9.com/test.css',fullscreen:true,beforeSetSource:ubb2html,beforeGetSource:html2ubb,focus:focusAction,blur:blurAction,forcePtag:true})[0].editor,sHtml;
editor.focus();
