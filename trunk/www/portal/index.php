...
function king_ajax_hit(){
	...
	//循环更新数据
	if($array['count']>55){//这个值过大的话，SQLite会出错。
		foreach($array as $key => $val){
			list($modelid,$kid)=explode('|',$key);//重新获得modelid和kid，和上面无任何关系
			$model=$king->portal->infoModel($modelid);
			$_array=array(
				'nhit'=>"[[nhit+{$val['count']}]]",
				'nhitlate'=>"[[(nhitlate*nhit+".(kc_now()*$val['count']).")/(nhit+{$val['count']})]]",
			);
	...
	if($kid){
		...
		if(in_array($king->user->userid,$editors) || (int)$id['userid']===(int)$king->user->userid){
			if(!in_array($king->user->userid,$editors)){//如果不是栏目管理员，则进行有效期验证
				if((kc_now() - $model['nlocktime']*3600 > $id['ndate']) && (int)$model['nlocktime']!==0){//如果超过可允许编辑时间期限 并 可编辑时间不能为0，则提示错误
...
*/
		...
		//更新时间
		$_array['nlastdate']=kc_now();
		//如果有kid1值，则对kid1对应的nlastdate进行更新
		if($kid1){
			$king->db->update('%s__'.$model['modeltable'],array('nlastdate'=>kc_now()),'kid='.$kid1.' limit 1');
		}
