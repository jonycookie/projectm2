...
function king_def(){
	...
	$s=$king->openList($_cmd,$_js,$king->db->pagelist('manage.content.php?pid=PID&rn=RN&listid='.$listid,$sql_count),array('listid'=>$listid,'kid1'=>$kid1));//&kid1={$kid1}
//	$s.="function ispath(){};";
...
*/
		...
		if($kid1){
			$king->db->update('%s__'.$model['modeltable'],array('nlastdate'=>kc_now()),'kid='.$kid1);
		}
		//图片框写远程路径的时候，抓图
