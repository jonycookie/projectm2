...
function king_ajax_express(){
	...
	if($GLOBALS['ischeck']){
		...
		if(empty($data['nsenddate'])) $array['nsenddate']=kc_now();//如果nsenddate为空，则填写当前时间戳
...
function king_def(){
	...
	foreach($res as $rs){//td
		$s.='ll('.$rs['oid'].',\''.$rs['ono'].'\',\''.$rs['nstatus'].'\',\''.$rs['kname'].'\','.$rs['nnumber'].',\''. ($rs['paymethod'] ? $king->lang->get('portal/orders/method/'.$rs['paymethod']) : '--') .'\','.$rs['ndate'].',\''.number_format($rs['ntotal'],2).'\',\''.number_format($rs['nexpress'],2).'\',\''.number_format($rs['ntotal']+$rs['nexpress'],2).'\',0);';
	}
