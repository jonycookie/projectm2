...
/*
	...
	$Id: cp_mtag.php 10887 2008-12-30 08:42:17Z zhengqingpeng $
*/
...
$fieldarr = $_SGLOBAL['profield'] = array();
...
if($_GET['op'] == 'manage') {
	...
	if(submitcheck('outsubmit')) {
		...
		showmessage('do_success', "space.php?uid=$_SGLOBAL[supe_uid]&do=mtag");
	}
	...
	if($tagid && submitcheck('pmsubmit')) {
		...
		$message = getstr(cplang('apply_mtag_manager', array($mtagurl, $mtag['tagname'], $_POST['message'])), 0, 1, 1, 1, 0, 1);
		...
		if(!$notearr) {
			...
			foreach ($_SGLOBAL['usergroup'] as $gid => $value) {
				if($value['managemtag']) $groups[] = $gid;
			}
			if($groups) {
