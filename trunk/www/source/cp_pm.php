...
/*
	...
	$Id: cp_pm.php 10836 2008-12-26 10:02:14Z liguode $
*/
...
if($_GET['op'] == 'checknewpm') {
	...
	if(submitcheck('pmsubmit')) {
		...
		if($touid) {
			...
			//发送邮件通知
			if($return > 0) {
				smail($touid, '', cplang('friend_pm',array($_SN[$space['uid']], getsiteurl().'space.php?do=pm')));
			}
			...
			if($return > 0) {
				...
				while($value = $_SGLOBAL['db']->fetch_array($query)) {
					...
					smail($value['uid'], '', cplang('friend_pm',array($_SN[$space['uid']], getsiteurl().'space.php?do=pm')));
				}
			}
		}
