...
/*
	...
	$Id: profile.php 10925 2009-01-06 03:01:49Z zhengqingpeng $
*/
	...
	if($residelist) {
		...
		foreach ($residelist as $key => $value) {
			$task['result'] .= '<li>
				...
				<p><a href="cp.php?ac=friend&op=add&uid='.$value['uid'].'" id="a_reside_friend_'.$key.'" onclick="ajaxmenu(event, this.id, 99999, \'\', -1)">加为好友</a></p>
				</li>';
		}
		$task['result'] .= '</ul>';
	}
	if($sexlist) {
		...
		foreach ($sexlist as $key => $value) {
			$task['result'] .= '<li>
				<div class="avatar48"><a href="space.php?uid='.$value['uid'].'" target="_blank"><img src="'.avatar($value['uid'], 'small').'" class="avatar"></a></div>
				<p><a href="space.php?uid='.$value['uid'].'" target="_blank">'.$_SN[$value['uid']].'</a></p>
				<p><a href="cp.php?ac=friend&op=add&uid='.$value['uid'].'" id="a_sex_friend_'.$key.'" onclick="ajaxmenu(event, this.id, 99999, \'\', -1)">加为好友</a></p>
				</li>';
		}
		$task['result'] .= '</ul>';
	}
