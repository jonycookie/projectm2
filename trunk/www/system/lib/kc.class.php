...
public function getModule(){
	...
	if($module){
		...
		if(!$res=$this->db->getRows("select kpath from %s_module where islock=0 order by norder,kid"))
			$res=array();
		$array=array();
		foreach($res as $rs){
			$array[]=$rs['kpath'];
...
*/
public function openList($_cmd=null,$_js=null,$_plist=null,$_ext=array()){
...
//	$s.=null;
	...
	//$this->mList.='if($.(\'#k_formlist\').list==undefined){$(\'k_formlist\').innerHTML=\'<table class="k_table_list" cellspacing="0" id="k_table_list"><tr><td class="red c" style="line-height:100px;">'.addslashes($this->lang->get('system/error/notrecord')).'</td></tr></table>\'}';
