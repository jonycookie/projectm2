...
function __autoload($class_name){
	...
	if(in_array($clsname,array('kc_template','kc_xml','kc_verify','kc_mysql','kc_sqlite'))){
		$fpath=KC_ROOT.'system/lib/'.strtolower(substr($clsname,3-strlen($clsname))).'.class.php';
	}else{
		$fpath=KC_ROOT.strtolower($clsname).'/core.class.php';
	}
...
//			}
		...
		case 21:$_is=preg_match("/(<(\/?)(script|base|iframe|style|html|body|title|link|meta|\?|\%)([^>]*?)>|(<[^>]*)on[a-zA-Z]+\s*=([^>]*>))/isU",$post);break;
...
function kc_grab_get($imgpath){
	...
	if($img=file_get_contents($imgpath)){
		...
		$date=gmdate('Y-m-d h:i',kc_now());
		list($msec,$sec)=explode(' ',microtime());
		$path=$king->config('uppath').'/image/'.gmdate('Y/m/d/h/i',kc_now()).'/'.kc_random(3).round($msec*1E6).'.'.$fext;
...
*/
function kc_f_ico($_name){
	switch(kc_f_ext($_name)){
		case 'doc':$icon='b3';break;
		case 'htm':$icon='f3';break;
		case 'html':$icon='f3';break;
		case 'js':$icon='l3';break;
		case 'mid':$icon='k3';break;
		case 'wma':$icon='k3';break;
		case 'mp3':$icon='k3';break;
		case 'pdf':$icon='j3';break;
		case 'php':$icon='a3';break;
		case 'jpg':$icon='d3';break;
		case 'png':$icon='i3';break;
		case 'gif':$icon='d3';break;
		case 'bmp':$icon='d3';break;
		case 'swf':$icon='h3';break;
		case 'txt':$icon='h6';break;
		case 'xls':$icon='c3';break;
		case 'zip':$icon='e3';break;
		case 'rar':$icon='e3';break;
		default:$icon='g3';
	}
	return $icon;
}
...
/*
			if(count($GLOBALS['file_get_contents_array'])>20){
...
function kc_now(){
	global $king;
	return time()+($king->config('timediff')*3600);
}
...
function kc_setCookie($name,$val,$expire = 0) {
	global $king;
	setcookie($name, $val,$expire ? kc_now() + $expire : 0, $king->config('inst'));
}
/**
...
function kc_pagelist($_url='',$_per,$_pid=1,$_rn=20,$_inner=null){
	...
	$previous=$_pid==1
		? '<span>&lt; Previous</span>'
		: '<a href="'.kc_formatpath($_url,$_pid-1).'">&lt; Previous</a>';
