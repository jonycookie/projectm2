...
function king_ajax_msg(){
	...
	if(!isset($s{0})){
		...
		foreach($res as $rs){
			...
			$s.='<i>['.gmdate('n-j, g:i A',$rs['ndate']).']</i></span></td></tr>';
		}
		$s.='</table>';
...
function king_ajax_brow_left(){
	...
	if(!$jscache){
		...
		foreach($folders as $val){
			...
			$jscache.="\$('#$obj').after(lll('$path','$val',$isSub,$space));\$.kc_ready('#k_brow_obj_"._path2id($path.$val)."');";

			//若在$path_def路径内的话，自动展开
			if(False!==strpos($path_def,$path.$val.'/')){
				$_id=_path2id($path.$val);
				if(!preg_match('/^\_+$/',$_id)){
					$jscache.="\$.kc_ajax({{$verbs},CMD:'brow_left',path:'$path$val/',ID:'k_brow_dir_{$_id}',obj:'k_brow_obj_{$_id}',space:".($space+1)."});";
				}
			}
		}
		...
		for($i=0;$i<5;$i++){
			...
			if($_FILES['file']['name'][$i]!=''){
				...
				if(in_array($ext,$_array_filetype)){
					kc_post('isrename')==1
						? $filename=kc_now().$i.'.'.$ext //这里定义的filename和$info数组中的filename不是一个对象
...
function king_log(){
	...
	foreach($_res as $_rs){	 //td
		$s.='ll('.$_rs['kid'].',\''.$_rs['adminname'].'\','.($_rs['nip']).','.$_rs['nlog'].','.$_rs['ndate'].',\''.addslashes(htmlspecialchars($_rs['ktext'])).'\',0);';//date('Y-m-d H:i:s',$_rs['logdate'])
	}
...
//		"checkself(K[0])",
	...
	foreach($_res as $_rs){	 //td
		$s.='ll('.$_rs['adminid'].',\''.$_rs['adminname'].'\','.($_rs['admincount']).','.$_rs['admindate'].',\''.$_rs['adminlevel'].'\',0);';
	}
...
*/
	...
	foreach($res as $rs){
		$s.='ll('.$rs['kid'].',\''.$rs['kpath'].'\',\''.$rs['adminname'].'\','.$rs['ndate'].',\''.kc_f_ico($rs['kpath']).'\',\''.$rs['ktitle'].'\',0);';
	}
	//结束列表
	$s.=$king->closeList();
...
function king_module(){
	...
	$_sql='select kid,kpath,ndbver,islock from %s_module order by norder';// where kid1=0
	if(!$res=$king->db->getRows($_sql,1))
		$res=array();
	...
	$s.='function islock(is){var I1;is?I1=\'b8\':I1=\'c8\';return I1;};';
		...
		$s.='ll(\''.$king->lang->get('system/module/name').'\',\''.$king->lang->get('system/module/path').'\',\'manage\',\''.$king->lang->get('system/module/sub').'\',\''.$king->lang->get('system/common/version').'\',1);';	//th
	...
	foreach($res as $rs){//td
		...
		$s.='ll('.$rs['kid'].',\''.$kname.'\',\''.($rs['kpath']).'\',\''.$subs.'\',\''.number_format($rs['ndbver']/100,2).'\','.$rs['islock'].',0);';
	}
...
*/
	...
	foreach($_res as $_rs){	 //td
		$s.='ll('.$_rs['kid'].',\''.addslashes($_rs['kfile']).'\','.($_rs['nline']).','.$_rs['ntype'].',\''.addslashes(kc_short($_rs['kmsg'],70,20)).'\','.$_rs['ndate'].',\''.$_rs['kurl'].'\',0);';
	}
