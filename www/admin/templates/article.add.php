...
<div class="container" id="cpcontainer">
  ...
  <form action="<?=__SELF__?>?do=article&operation=post" method="post" enctype="multipart/form-data" name="savearticle" id="savearticle">
    ...
    <tbody id="base">
      <tr>
        <td>栏目：</td>
        <td colspan="3"><?php if($cata_option){?><select name="catalog" id="catalog" style="width:auto;">
      ...
      <tr>
        ...
        <td colspan="3" style="width:auto"><input type="text" name="title" class="txt" id="title" value="<?=$rs['title']?>" style="width:520px"/></td>
    </td>
      </tr>
      <tr>
        <td>短标题：</td>
        <td colspan="3" style="width:auto"><input name="stitle" class="txt" id="stitle" value="<?=$rs['stitle']?>"  style="width:520px" type="text"/></td>
      </tr>
      <tr>
        <td>出处：</td>
...
<ul><li onClick="showDialog('<?=__SELF__?>?do=dialog&operation=Aupload','pic',600,140);$('.close').click();">本地上传</li><li onClick="showDialog('<?=__SELF__?>?do=dialog&operation=file&hit=file&type=gif,jpg,png,bmp,jpeg','pic',600,500);$('.close').click();">从网站选择</li><li onClick="showPic('pic');$('.close').click();">查看缩略图</li><li onClick="cutPic('pic');$('.close').click();">剪裁</li></ul></div>
      ...
      <tr>
        <td>副标题：</td>
        <td colspan="3" style="width:auto"><input name="subtitle" class="txt" id="subtitle" value="<?=$rs['subtitle']?>"  style="width:520px" type="text"/></td>
      </tr>
      <tr>
        <td>内容：</td>
        <td colspan="3" style="width:auto"><span style="float:right;"><img src="<?=$iCMS->dir?>admin/images/add.gif" onclick="setEditorSize('+','content',200)" title="增加编辑器高度"/> <img src="<?=$iCMS->dir?>admin/images/desc.gif" onclick="setEditorSize('-','content',200)" title="减少编辑器高度"/></span><input name="remote" type="checkbox" class="checkbox" id="remote" value="1" <?php if($iCMS->config['remote']=="1")echo 'checked="checked"'?>/>下载远程图片
    <!--<input name="dellink" type="checkbox" id="dellink" value="1" />删除非站内链接 -->
      ...
      <tr>
        <td>摘要：</td>
        <td colspan="3"><textarea name="description" id="description" onKeyUp="textareasize(this)" class="tarea" style="width:460px"><?=$rs['description']?></textarea></td>
      </tr>
      <tr>
        <td>关键字：</td>
        <td colspan="3"><input name="keywords" class="txt" type="text" id="keywords" value="<?=$rs['keywords']?>" style="width:460px"/> 多个关键字请用,格开</td>
      </tr>
      <tr>
        <td>标签：</td>
        <td colspan="3" style="width:auto"><input name="tag" class="txt" type="text" id="tag" size="50" value="<?=$rs['tags']?>" /><input name="keywordToTag" type="checkbox" class="checkbox" id="keywordToTag" value="1" />
...
<input type="button" onclick="editContentLink('related')" value="..." tabindex="13" class="btn"/>
      ...
      <tr>
        <td>虚链接：</td>
        <td colspan="3" style="width:auto"><select name="vlink[]" size="10" multiple="multiple" id="vlink">
      ...
      <tr>
        <td>属性：</td>
        <td colspan="3" style="width:auto"><select name="type[]" size="10" multiple="multiple" id="type">
      ...
      <tr>
        <td>置顶权重：</td>
        <td colspan="3"><input id="top" class="txt" value="<?=$rs['top']?>" name="top" type="text"/></td>
      </tr>
      <tr>
        <td>发布时间：</td>
        <td colspan="3"><input id="pubdate" class="txt" value="<?=$rs['pubdate']?>" name="pubdate" type="text" onclick="showcalendar(event, this)" style="width:208px"/></td>
      </tr>
      <tr>
        <td>模板：</td>
        <td colspan="3" style="width:auto"><input id="template" class="txt" value="<?=$rs['tpl']?>" name="template" type="text"/> <img src="admin/images/selecttpl.gif" width="67" height="19" align="absmiddle" onclick="showDialog('<?=__SELF__?>?do=dialog&operation=template&hit=file&type=htm','template');"/></td>
      </tr>
      <tr>
        <td>自定链接：</td>
        <td colspan="3" style="width:auto"><input name="customlink" class="txt" type="text" id="customlink" value="<?=$rs['customlink']?>" />
        <br />
        只能由英文字母、数字或_-组成(不支持中文),留空则自动以标题拼音填充</td>
      </tr>
      <tr>
        <td>外部链接：</td>
        <td colspan="3" style="width:auto"><input name="url" class="txt" type="text" id="url" size="50" value="<?=$rs['url']?>" /> 
        不填写请留空.</td>
      </tr>
       </tbody>
      <!--tr class="nobg"><td colspan="4">
		<table width="100%" cellspacing="0" cellpadding="0">
			...
			<tbody id="attachbodyhidden" style="display:none"><tr><th><input type="file" name="attach[]" class="uploadbtn"/><span id="localfile[]"></span><input type="hidden" name="localid[]" /></th><th><input type="text" name="attachdesc[]" class="txt" /></th></tr></tbody>
