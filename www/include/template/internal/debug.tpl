...
{else}
	...
	<tr bgcolor=#cccccc><td colspan=2 align="left"><b>Included templates & config files (load time in seconds):</b></td></tr>
	{foreach key=key value=templates from=$_debug_tpls}
	...
	{foreach key=key value=vars from=$_debug_keys}
		<tr bgcolor={if $key % 2}#eeeeee{else}#fafafa{/if}>
		<td valign=top align="left"><tt><font color=blue>{ldelim}${$_debug_keys[$key]}{rdelim}</font></tt></td>
