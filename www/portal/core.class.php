...
public function depathMode($info){
	...
	$date=explode(',',gmdate('Y,m,d,h,i,s,M',kc_now()));
...
//			$c[]=array($f,9);
			...
			$date=$data[$f]=='TODAY'?gmdate( "Y-m-d"):$data[$f];
...
public function lastUpdated($listid,$type=null){
	...
	if($type=='page'||$type=='list'){
		$king->db->update('%s_list',array('nupdate'.$type=>kc_now()),"listid=$listid");
	}else{
		$king->db->update('%s_list',array('nupdatepage'=>kc_now(),'nupdatelist'=>kc_now()),"listid=$listid");
	}
}
/**
	Model解压
