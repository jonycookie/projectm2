...
function king_ajax_login(){
	...
	*/
		...
		if($king->user->isuc){//如果有UC
				...
				if($array_uc[0]<0){//用户名不存在或密码错误
					...
					if($res=$king->db->getRows_one("select userpass,ksalt,userid from %s_user where username='".$king->db->escape($username)."'")){//如果有这么个账号
						...
						if(md5($res['ksalt'].$data['userpass'])!=$res['userpass']){//若不一致，则进行更新
							...
							$king->db->update('%s_user',array('userpass'=>$userpass,'lastlogindate'=>kc_now()),'username=\''.$king->db->escape($username.'\''));
						}else{
							$king->db->update('%s_user',array('lastlogindate'=>kc_now()),'username=\''.$king->db->escape($username.'\''));
						}
					}else{//如果本地没有这个账号，则添加
						$usermail=$array_uc[3];
			...
			if($res=$king->db->getRows_one("select userpass,ksalt,userid from %s_user where username='".$king->db->escape($username)."' and isdelete=0")){//如果有这么个账号
				...
				if($md5pass!=$res['userpass']){//若不一致，提示错误
					...
					$userid=$res['userid'];
					$king->db->update('%s_user',array('lastlogindate'=>kc_now()),'userid='.$userid);
				}
			}else{
				$is=True;
				$errId=-1;
	...
	if($GLOBALS['ischeck']){
		...
		//登录日志?
		kc_ajax($king->lang->get('portal/user/loginok'),$s."<p class=\"k_ok\">".$king->lang->get('portal/user/welcome')."</p>",1,$js);
...
function king_ajax_logout(){
	...
	kc_ajax($king->lang->get('user/common/logout'),$s."<p class=\"k_ok\">".$king->lang->get('user/ok/logout')."</p>",0,'parent.location=\'../user/login.php\'');
...
function king_ajax_register(){
	...
	if($GLOBALS['ischeck']){
		...
		$array['uid']=$uid;
		$array['ksalt']=$salt;
		$array['regdate']=kc_now();
		$userid=$king->db->insert('%s_user',$array);

		//写Cookie
		$s=$king->user->userLogin($userid,2592000);

		kc_ajax($king->lang->get('system/common/welcome'),$s."<p class=\"k_ok\">".$king->lang->get('portal/user/regok')."</p>",1,$js);
	}
...
//		kc_error(nl2br(print_r($user,1)));
		...
		kc_ajax('OK',$s."<p class=\"k_ok\">".$king->lang->get('system/ok/set')."</p>",1,$js);
	}
...
function king_ajax_lostpwd1(){
	...
	if($GLOBALS['ischeck']){
		...
		kc_ajax($king->lang->get('system/common/welcome'),$s."<p class=\"k_ok\">".$king->lang->get('portal/user/lostok')."</p>",0,$js);
	}
